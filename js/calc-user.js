  ...
  var l4u__LAMBDA = {1:1.0,2:1.16,3:1.29,4:1.36};
  function l4u__calcEarth(soilId, acCabS, kva){
    ...
    var lam = l4u__LAMBDA[n] || 1.36;
    var rn = +((r1*lam)/n).toFixed(2);
    var condS = acCabS <= 16 ? acCabS : Math.max(16, Math.ceil(acCabS/2));
    var condRun = 5;
    return {
      soil: soil.label, rho: rho, rods: n, rn: rn, warn: rn > 5.0,
      condS: condS, condRun: condRun,
      rodCost: n*l4u_PC.earthRod.p, condCost: n*condRun*l4u_PC.earthCond.p, pitCost: n*l4u_PC.earthPit.p,
      total: n*l4u_PC.earthRod.p + n*condRun*l4u_PC.earthCond.p + n*l4u_PC.earthPit.p
    };
  }
  ...
  function l4u__renderEarth(s){
    ...
    var warnHtml = e.warn
      ? '<div style="background:#fff7ed;border:1px solid #fb923c;border-radius:8px;padding:10px 12px;margin-top:10px;font-size:12px;color:#9a3412;line-height:1.5">'+
          ...
          'Consider chemical earthing compound (bentonite/salt-charcoal backfill) around each rod.'+
        '</div>'
      : '';
    el.innerHTML=
