  ...
  // ── DATABASES ────────────────────────────────────────────────────────────
  const l4u_INVERTER_DB = (typeof EG_INVERTERS !== 'undefined') ? EG_INVERTERS : [];
  const l4u_BATTERY_DB  = (typeof EG_BATTERIES !== 'undefined') ? EG_BATTERIES : {12:[],24:[],48:[]};
  ...
  function l4u__runEngine(apps){
    ...
    // 950W/kVA derating — correct for modern hybrid MPPT inverters (~0.95 PF)
    var reqKva=Math.max((tw*1.25)/950, surge/1600);
    var DB_MAX_KVA=l4u_INVERTER_DB[l4u_INVERTER_DB.length-1].kva; // 20kVA
    var oversizeKva=(reqKva>DB_MAX_KVA); // true when load exceeds single-unit ceiling
    ...
    // Rule: if required kWh exceeds the ceiling of the current voltage family,
    // step up to the next voltage tier (keeping kVA as close as possible).
    ...
    if(V===12 && reqKwh>V12_CEILING){
      // Battery need exceeds 12V pack ceiling — step to smallest 24V inverter
      var inv24=l4u_INVERTER_DB.filter(function(i){return i.v===24;}).find(function(i){return i.kva>=inv.kva;})||l4u_INVERTER_DB.filter(function(i){return i.v===24;})[0];
    ...
    if(V===24 && reqKwh>V24_CEILING){
      // Battery need exceeds 24V pack ceiling — step to 48V at same or nearest kVA
      var inv48=l4u_INVERTER_DB.filter(function(i){return i.v===48;}).find(function(i){return i.kva>=inv.kva;})||l4u_INVERTER_DB.filter(function(i){return i.v===48;})[0];
      if(inv48){ inv=inv48; V=48; acA=(inv.kva*1000)/230; conflict=true; }
      rawAh=dw/(V*DOD*EFF);
      reqKwh=rawAh*V/1000;
    }
    var fam=l4u_BATTERY_DB[V]||[];
    var opts=[];
    var selPack=null;
    var MULTI_PACK_CEILING=30.00; // largest confirmed single pack in Nigerian market
    if(V===12){
      // 12V: find smallest pack where ah >= rawAh
      ...
      // Exceeds largest single pack — show raw requirement, defer to installer
      ...
      // 24V/48V: find smallest single pack covering reqKwh
    ...
    var PSH=5,PR=0.85,PW=l4u_PANEL.wattage,Vmp=l4u_PANEL.vmp,Imp=l4u_PANEL.imp;
    ...
    return {
      ...
      inv:inv, fpV:fpV, V:V, acA:acA, conflict:conflict, reqKva:reqKva, oversizeKva:oversizeKva,
  ...
  // Builds the numeric "show your working" trail for the voltage-escalation
  // badge. Uses only the real numbers already computed by the engine and
  // the real ceiling constants (V12_CEILING=4.00, V24_CEILING=7.68) — these
  // ceilings are NOT a universal electrical standard, they are simply the
  // kWh rating of the largest battery pack we currently stock at that
  // voltage. The text below says so explicitly rather than implying a
  // physics limit, so a professional checking the app's math against their
  // own hand calculation can see exactly where each number came from.
  function l4u__buildConflictDetail(s){
    var lines=[];
    lines.push('<strong>Your numbers:</strong> your appliances need about '+s.reqKwh.toFixed(2)+'kWh of battery storage per day.');
    if(s.fpV===12){
      lines.push('At 12V, the largest battery pack we list holds 4.00kWh — not enough, so the system moved up to 24V.');
    }
    if(s.V===48 && (s.fpV===12||s.fpV===24)){
      lines.push('At 24V, the largest battery pack we list holds 7.68kWh — still not enough for '+s.reqKwh.toFixed(2)+'kWh, so the system moved up to 48V.');
    }
    lines.push('<strong>Important:</strong> 4.00kWh and 7.68kWh are not electrical limits — they\'re simply the biggest 12V and 24V packs currently in our product list. A bigger pack at the same voltage could exist; we haven\'t listed one yet.');
    lines.push('Once the voltage moved to '+s.V+'V, the smallest inverter available at that voltage was '+s.inv.kva+'kVA — which is why the inverter is larger than the appliance load alone (~'+s.reqKva.toFixed(1)+'kVA) would suggest.');
    return lines.map(function(l){return '<div style="margin-bottom:6px;">'+l+'</div>';}).join('');
  }

  window.l4u_toggleConflictDetail=function(){
    var el=document.getElementById('l4u-r-conflict-detail');
    var label=document.getElementById('l4u-r-conflict-toggle-label');
    if(!el||!label)return;
    var open=el.style.display!=='none';
    el.style.display=open?'none':'block';
    label.textContent=open?'Show the numbers':'Hide the numbers';
  };
  ...
  function l4u__renderResults(s){
    ...
    l4u__applyPanelMode(s,'C'); // Battery-led sizing — Mode C
    var b0=s.opts[s.selBat||0]||s.opts[0]||{};
    // Simple cards
    set('l4u-s-inverter-kva',s.inv.kva+' kVA · '+s.V+'V');
    ...
    set('l4u-s-battery-main',b0.multiPack?('~'+b0.reqKwh+'kWh required'):((b0.units||1)+' × '+(b0.label||'-')));
    set('l4u-s-battery-config',b0.multiPack?'Multi-pack — installer will advise':(b0.wiring||(b0.label||'-')));
    set('l4u-s-battery-alt',b0.multiPack?('~'+b0.reqKwh+' kWh total'):((b0.totalKwh||'-')+' kWh total'));
    set('l4u-s-panels-mode','Battery-led sizing • '+N(s.reqW)+'W required');
    // Load
    ...
    // Conflict badge — plain-language summary always shown; detailed
    // numeric trail available on click (for professional testers to
    // verify the math), collapsed by default for normal users.
    var badge=document.getElementById('l4u-r-conflict-badge'),ct=document.getElementById('l4u-r-conflict-text');
    var detailEl=document.getElementById('l4u-r-conflict-detail');
    if(badge&&ct){
      if(s.conflict){
        ct.textContent='Your battery bank is large enough that it needs a bigger voltage system to run safely — that\'s why your inverter is bigger than your appliances alone would suggest.';
        badge.style.display='block';
        if(detailEl){ detailEl.innerHTML=l4u__buildConflictDetail(s); }
      } else {
        badge.style.display='none';
      }
    }
    // Oversize banner — shown when load exceeds 20kVA single-unit ceiling
    var ob=document.getElementById('l4u-r-oversize-banner');
    if(ob){
      if(s.oversizeKva){
        var ciSuggest=s.reqKva>=30?'<div style="margin-top:8px;padding-top:8px;border-top:1px solid rgba(99,102,241,0.3);font-size:11px;color:#a5b4fc;">⚡ At '+s.reqKva.toFixed(1)+'kVA this system is in three-phase territory. <button onclick="egCIHandoff('+s.reqKva.toFixed(1)+','+((s.dw/1000).toFixed(1))+')" style="background:#6366f1;border:none;border-radius:6px;color:white;font-size:11px;font-weight:700;padding:4px 10px;cursor:pointer;margin-left:4px;">Open C&amp;I Calculator →</button></div>':'';
        ob.innerHTML='<strong style="display:block;margin-bottom:4px">🔀 Parallel Inverter Configuration Required</strong>'
          +'Your load requires ~<strong>'+s.reqKva.toFixed(1)+'kVA</strong>. This exceeds the largest single-unit inverter available. '
          +'Your installer will specify the parallel configuration and battery bank arrangement.'
          +ciSuggest;
    ...
    if(bd){var o=s.opts[0];if(!o){bd.innerHTML='';}else if(o.multiPack){
      bd.innerHTML='<div class="battery-card" style="border:2px solid #f59e0b;grid-column:1/-1;">'
        +'<div style="font-size:11px;font-weight:700;color:#f59e0b;text-transform:uppercase;letter-spacing:.8px;margin-bottom:6px;">Large System</div>'
        +'<div class="bval" style="font-size:28px;font-weight:800;">~'+o.reqKwh+' kWh</div>'
        ...
        +'This load requires more than 30kWh of battery storage. '
        +'Your installer will specify the exact pack arrangement (e.g. 2 × 20kWh, 3 × 15kWh) '
        +'based on available stock and site conditions. '
        +'For very large systems, consider the C&amp;I portal.'
        +'</div>'
        +'</div>';
    }else{bd.innerHTML='<div class="battery-card" style="border:2px solid var(--sun);grid-column:1/-1;">'+'<div class="bval" style="font-size:18px;">'+o.units+' ×</div>'+'<div style="font-size:11px;color:var(--sun);font-weight:700;margin:2px 0">'+o.label+'</div>'+'<div class="bunit">'+o.totalKwh+' kWh total</div>'+'<div style="font-size:11px;color:var(--muted);margin-top:4px">'+o.wiring+'</div>'+(o.warn?'<div style="font-size:10px;color:#ef4444;margin-top:4px">⚠ '+o.warn+'</div>':'')+'</div>';}}
    // Solar
    ...
    var tog=document.getElementById('l4u-tech-toggle'); if(tog)tog.style.display='block';
    // Init toggle state (hidden by default for consumer portal)
    if(typeof egInitTechSpec==='function') egInitTechSpec('l4u');
  ...
  function l4u__applyPanelMode(s,mode){
    ...
    s.gen=+((panels*l4u_PANEL.wattage*(s.PSH||5)*(s.PR||0.85))/1000).toFixed(2);
    return s;
  }
  ...
  function l4u__renderEarth(s){
    ...
    if(!s.earth){
      ...
      if(typeof egSetSimpleEarthWarn==='function') egSetSimpleEarthWarn('l4u',false,'');
      return;
    }
    var e=s.earth;
    // Populate simple-view earthing warning
    if(typeof egSetSimpleEarthWarn==='function') egSetSimpleEarthWarn('l4u',!!e.warn,e.soil||'');
    var warnHtml=e.warn
  ...
  function l4u_egPickBat(i){
    ...
    if(b){function set(id,v){var el=document.getElementById(id);if(el)el.textContent=v;}set('l4u-s-battery-main',b.multiPack?('~'+b.reqKwh+'kWh required'):((b.units||1)+' × '+b.label));set('l4u-s-battery-config',b.multiPack?'Multi-pack — installer will advise':b.wiring);set('l4u-s-battery-alt',b.multiPack?('~'+b.reqKwh+' kWh total'):(b.totalKwh+' kWh total'));}
      ...
      '<div style="background:#1e293b;border:1px solid #334155;border-radius:18px;padding:24px 20px;width:100%;max-width:320px;box-shadow:0 20px 60px rgba(0,0,0,.5);">'+
        ...
        (p.dutyCycle!=null?
          ...
          '<div style="display:flex;align-items:center;gap:12px;margin-bottom:20px;">'+
            '<button style="width:44px;height:44px;border-radius:12px;border:1px solid #334155;background:#0f172a;color:#f1f5f9;font-size:22px;cursor:pointer;display:flex;align-items:center;justify-content:center;font-weight:300;flex-shrink:0;" onclick="l4u_hrsStep(-1)">−</button>'+
  ...
  function l4u_qtyConfirm(name){
    ...
    var p=l4u_getAppliancePreset(name);if(!p)return;
    var hrs=hrsInp?Math.min(24,Math.max(0.5,parseFloat(hrsInp.value)||l4u_getDefaultHours(p.category))):l4u_getDefaultHours(p.category);
    l4u_qtyCancel();
    var newId=Date.now()+Math.random();
    l4u_appliances.push({id:newId,name:p.name,watts:p.watts,qty:qty,hours:hrs,inductive:p.inductive,dutyCycle:p.dutyCycle});
    l4u_renderList();
  ...
  function l4u_updateHours(id,val){var h=Math.min(24,Math.max(0.5,parseFloat(val)||1));var a=l4u_appliances.find(x=>x.id===id);if(a){a.hours=h;}l4u_renderList();}
  ...
  function l4u_renderList(){
    ...
    list.innerHTML=l4u_appliances.map(a=>{var effH=a.dutyCycle?24*a.dutyCycle:a.hours;var d=(a.watts*a.qty*effH/1000).toFixed(2);var hoursDisplay=a.dutyCycle?'24h ('+Math.round(a.dutyCycle*100)+'% duty)':'<input type="number" min="0.5" max="24" step="0.5" value="'+a.hours+'" style="width:42px;text-align:center;background:#0f172a;border:1px solid #475569;border-radius:6px;color:#f1f5f9;font-size:12px;padding:2px 4px;" onchange="l4u_updateHours('+a.id+',this.value)" onclick="event.stopPropagation()">h/day';return '<div class="appliance-item"><div class="app-info"><div class="app-name">'+a.name+'</div><div class="app-details"><span>'+a.watts+'W</span> × <span>'+a.qty+'</span> · <span>'+hoursDisplay+'</span></div></div><div style="display:flex;align-items:center;gap:12px"><div class="app-daily"><strong>'+d+'</strong> kWh/day</div><button class="btn-remove" onclick="l4u_removeAppliance('+a.id+')">✕</button></div></div>';}).join('');
  }
  ...
  function l4u_calculate(){if(!l4u_appliances.length)return;
    ...
    l4u__egS=l4u__runEngine(l4u_appliances);l4u__applyPanelMode(l4u__egS,'C');l4u__egS=l4u__buildBOM(l4u__egS,0);l4u__renderResults(l4u__egS);
    try {
      ...
      // Call through window so any wrappers (e.g. corporate.js agent mode) fire correctly
      if (typeof window.receiveEmbeddedCalculation === 'function') {
        window.receiveEmbeddedCalculation('user', flat, l4u_appliances);
      } else {
