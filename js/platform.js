// EG-VERSION: v304-fix
    ...
    function updateUserAccountUI() {
      ...
      // Treat gate-registered users the same as signed-in users for UI purposes
      const gateCompleted = (typeof window.egUserGateCompleted === 'function') && window.egUserGateCompleted();

      if (isSignedInEndUser() || gateCompleted) {
        if (guestBanner)  guestBanner.style.display  = 'none';
        if (calcBackBar)  calcBackBar.style.display   = 'none'; // hide back bar too — they came from gate
