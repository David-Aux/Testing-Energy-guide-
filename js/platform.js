// EG-VERSION: v304-fix
    ...
    function handleUserCalcResult(result, apps = []) {
      ...
      // Scroll results into view after forceResetScroll settles (rAF + 40ms + 120ms)
      setTimeout(function(){
        var res = document.getElementById('l4u-results');
        if (!res) return;
        var rp = document.querySelector('#user-calculator .right-panel');
        if (rp) { rp.scrollTo({ top: res.offsetTop - 16, behavior: 'smooth' }); }
        else { res.scrollIntoView({ behavior: 'smooth', block: 'start' }); }
      }, 160);
