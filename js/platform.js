// EG-VERSION: v304-fix
    ...
    async function saveCompleteInstallerProfile() {
      ...
      try {
        ...
        showToast('Profile saved! Your account is under review — you have full dashboard access in the meantime.', 'success');
        showScreen('installer-dashboard');
      } catch(e) {
        showLoading(false);
    ...
    function openUserCostScreen() {
      ...
      if (!egRequireUserAuth('Create a free account to view your full cost breakdown and connect with installers and vendors near you.')) return;
    ...
    async function updateVendorDashboard() {
      ...
      if (reqEl && supabaseClient) {
        try {
          ...
          let query = supabaseClient.from('leads').select('*', { count: 'exact', head: true }).in('status', ['vendor_requested', 'vendor_responded']);
    ...
    async function renderVendorRequests() {
      ...
      container.innerHTML = data.map(lead => {
        ...
        const extras = [
          ...
          lead.contact_method     ? `Preferred contact: <strong>${lead.contact_method}</strong>` : null,
        ].filter(Boolean).join(' &nbsp;·&nbsp; ');
        ...
        if (isClosed) {
          ...
          buttons = `
            <button class="btn btn-warning small-btn" onclick="openOfferBuilderFromLead('${leadEnc}')" style="white-space:nowrap;">📦 Build Offer</button>
            <button class="btn btn-outline small-btn" onclick="contactCustomer('${phone}')" style="white-space:nowrap;">📞 Call</button>
            ${email ? `<button class="btn btn-outline small-btn" onclick="contactLeadEmail('${email}')" style="white-space:nowrap;">✉️ Email</button>` : ''}`;
        }
