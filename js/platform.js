// EG-VERSION: v304-fix
    ...
    function showScreen(screenId) {
      ...
      if (screenId === 'admin-panel') { egAdminSetTab('pending'); egAdminSetRole('all'); if (egIsAdmin()) egLoadAdminPanel(); }
    ...
    async function restoreEnergyGuideUserSession(sessionArg = null) {
      ...
      try {
        ...
        if (profile) {
          ...
          updateUserAccountUI();
          // Route returning user directly to their portal
          const role = profile.role || currentRole;
          if (window._adminRouteRequested && profile.is_admin) {
    ...
    (async () => {
      try {
        ...
        if (urlParams.get('view') === 'admin') {
          window._adminRouteRequested = true;
        }
        if (!supabaseClient) return;
