// EG-VERSION: v304-fix
    ...
    let lastSubmittedLeadId = null;
    let lastSubmittedLeadMeta = null; // { targetName, targetType, products, timestamp }
    ...
    async function egLoadVendorProductsInline(vendorId) {
      ...
      try {
        ...
        container.innerHTML = data.map(p => {
          ...
          return `
          <div style="padding:10px 0;border-bottom:1px solid #1e2d42;font-size:13px;">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;">
              <div style="flex:1;">
                <div style="font-weight:600;color:#f1f5f9;">${safeText(p.name || 'Unnamed product')}</div>
                <div style="color:#6b7280;font-size:12px;">${safeText(p.category || '')}${specLabel ? ' · ' + safeText(specLabel) : ''}${p.brand ? ' · ' + safeText(p.brand) : ''}</div>
              </div>
            ...
            <button onclick="selectVendorProductForOffer('${safeText(vendorId)}','${safeText(p.id)}','${escapedName}',${escapedPrice})"
              style="margin-top:8px;width:100%;padding:8px;background:linear-gradient(135deg,#F59E0B,#D97706);
                     border:none;border-radius:7px;color:white;font-size:12px;font-weight:700;cursor:pointer;">
              Request Offer for this product
            </button>
          </div>`;
        }).join('');
      } catch(e) {
    ...
    async function submitLead() {
      ...
      try {
        ...
        if (result.success) {
          // Capture meta for the Track This Request flow
          lastSubmittedLeadId = null;
          lastSubmittedLeadMeta = {
            targetType: isVendor ? 'vendor' : 'installer',
            targetName: isVendor
              ? (selectedVendorProfile ? getVendorDisplayName(selectedVendorProfile) : 'Vendor')
              : (selectedInstallerProfile ? getInstallerDisplayName(selectedInstallerProfile) : 'Installer'),
            products: isVendor ? selectedProducts.map(p => p.name) : [],
    ...
    function showScreen(screenId) {
      ...
      if (screenId === 'user-my-requests') renderMyRequests();
    ...
    function updateLeadSuccessAccountUI() {
      ...
      if (isSignedInEndUser()) {
        ...
        text.textContent = 'Your account is connected. View your submitted requests and their status anytime.';
        primaryBtn.textContent = '📍 View My Requests';
        primaryBtn.onclick = () => openMyRequests();
    ...
    async function renderMyRequests() {
      ...
      if (!container) return;
      container.innerHTML = '<div class="info">Loading your requests...</div>';

      try {
        const userId = currentUser?.id;
        if (!userId) {
          container.innerHTML = '<div class="info">Sign in to view your requests.</div>';
          return;
        }

        const { data: leads, error } = await supabaseClient
          .from('leads')
          .select('id, created_at, project_type, status, vendor_quote_type, note, state, city, full_name')
          .eq('user_id', userId)
          .order('created_at', { ascending: false });

        if (error) {
          // user_id column may not exist yet — show friendly message
          container.innerHTML = `
            <div class="info" style="text-align:center;padding:32px 16px;">
              <div style="font-size:32px;margin-bottom:12px;">⚙️</div>
              <div style="font-weight:600;margin-bottom:6px;color:#f1f5f9;">Setup needed</div>
              <div style="font-size:13px;color:#94a3b8;">Run the database migration in Supabase to enable request tracking.</div>
            </div>`;
          return;
        }
        ...
        if (!leads || leads.length === 0) {
          container.innerHTML = `
            <div class="info" style="text-align:center;padding:32px 16px;">
              ...
              <div style="font-weight:600;margin-bottom:6px;color:#f1f5f9;">No requests yet</div>
              <div style="font-size:13px;color:#94a3b8;">When you send a request to a vendor or installer, it will appear here.</div>
            </div>`;
          return;
        }

        const statusLabel = {
          'vendor_requested': { text: 'Pending', color: '#F59E0B', bg: 'rgba(245,158,11,0.12)' },
          'open':             { text: 'Pending', color: '#F59E0B', bg: 'rgba(245,158,11,0.12)' },
          'claimed':          { text: 'In Progress', color: '#0a7ea4', bg: 'rgba(10,126,164,0.12)' },
          'closed':           { text: 'Closed', color: '#94a3b8', bg: 'rgba(148,163,184,0.12)' },
        };

        container.innerHTML = leads.map(lead => {
          const isVendor = lead.project_type === 'vendor_request';
          const typeLabel = isVendor ? '🏪 Vendor Request' : '🛠 Installer Request';
          const s = statusLabel[lead.status] || { text: lead.status || 'Submitted', color: '#94a3b8', bg: 'rgba(148,163,184,0.12)' };
          const dateStr = lead.created_at
            ? new Date(lead.created_at).toLocaleDateString('en-NG', { day:'numeric', month:'short', year:'numeric', hour:'2-digit', minute:'2-digit' })
            : '';
          const products = lead.vendor_quote_type
            ? lead.vendor_quote_type.split(',').map(p => p.trim()).filter(Boolean)
            : [];

          return `
            <div style="background:#141e2e;border:1.5px solid #1e2d42;border-radius:14px;padding:16px;margin-bottom:12px;">
              <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">
                <div style="font-size:12px;color:#94a3b8;font-weight:600;">${typeLabel}</div>
                <div style="font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;
                            color:${s.color};background:${s.bg};">${s.text}</div>
              </div>
              ${products.length > 0 ? `
                <div style="margin-bottom:8px;">
                  ${products.map(p => `<div style="font-size:13px;font-weight:600;color:#f1f5f9;margin-bottom:2px;">• ${safeText(p)}</div>`).join('')}
                </div>` : ''}
              ${lead.state || lead.city ? `
                <div style="font-size:12px;color:#94a3b8;margin-bottom:6px;">
                  📍 ${[lead.city, lead.state].filter(Boolean).join(', ')}
                </div>` : ''}
              <div style="font-size:11px;color:#475569;margin-top:8px;border-top:1px solid #1e2d42;padding-top:8px;">
                Submitted: ${dateStr}
              </div>
            </div>`;
        }).join('');
        ...
        console.error('renderMyRequests error:', e);
        container.innerHTML = '<div class="info" style="color:#b91c1c;">Could not load your requests right now. Please try again.</div>';
      }
    }
