// EG-VERSION: v304-fix
    ...
    async function egLoadVendorProductsInline(vendorId) {
      ...
      try {
        ...
        container.innerHTML = `<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:4px;">` +
        data.map(p => {
          ...
          const imgHtml = p.image_url
            ? `<img src="${safeText(p.image_url)}" alt="${safeText(p.name||'')}" style="width:100%;height:80px;object-fit:cover;border-radius:8px 8px 0 0;display:block;">`
            : `<div style="width:100%;height:80px;background:linear-gradient(135deg,#1e2d42,#0f1722);border-radius:8px 8px 0 0;display:flex;align-items:center;justify-content:center;font-size:28px;">${typeIcons[p.category]||'📦'}</div>`;
          ...
          return `
          <div style="background:#0a1628;border:1px solid #1e2d42;border-radius:10px;overflow:hidden;display:flex;flex-direction:column;cursor:pointer;"
               onclick="egShowProductDetail(JSON.parse(this.dataset.product), '${safeText(vendorId)}')"
               data-product="${detailJson}">
            ${imgHtml}
            <div style="padding:8px;flex:1;display:flex;flex-direction:column;gap:2px;">
              <div style="font-weight:700;color:#f1f5f9;font-size:12px;line-height:1.3;">${safeText(p.name||'Unnamed')}</div>
              ${specLabel ? `<div style="color:#64748b;font-size:10px;">${safeText(specLabel)}</div>` : ''}
              <div style="font-weight:800;color:#fbbf24;font-size:13px;margin-top:2px;">₦${Number(p.price||0).toLocaleString()}</div>
              <button onclick="event.stopPropagation();selectVendorProductForOffer('${safeText(vendorId)}','${safeText(p.id)}','${escapedName}',${escapedPrice})"
                style="margin-top:6px;width:100%;padding:6px 4px;background:linear-gradient(135deg,#F59E0B,#D97706);border:none;border-radius:6px;color:white;font-size:11px;font-weight:700;cursor:pointer;">
                Get Offer
              </button>
            </div>
          </div>`;
    ...
    function renderVendorMarketplaceCards(profiles) {
      return (profiles || []).map(profile => {
        ...
        const catChips = (categories||'').split(',').map(c=>c.trim()).filter(Boolean).map(c =>
          `<span style="display:inline-block;padding:3px 8px;background:rgba(251,191,36,0.12);color:#fbbf24;font-size:10px;font-weight:700;border-radius:20px;border:1px solid rgba(251,191,36,0.25);margin:2px 2px 0 0;">${safeText(c)}</span>`
        ).join('');
        const installBadge = (profile.offers_installation === 'yes' || profile.offers_installation === true)
          ? `<span style="display:inline-flex;align-items:center;gap:3px;padding:3px 8px;background:rgba(34,197,94,0.12);color:#22c55e;font-size:10px;font-weight:700;border-radius:20px;border:1px solid rgba(34,197,94,0.25);margin:2px 2px 0 0;">🔧 Installs</span>` : '';
        const brands = profile.brands_carried ? `<div style="color:#94a3b8;font-size:11px;margin-top:8px;line-height:1.4;"><span style="color:#64748b;">Brands: </span>${safeText(profile.brands_carried)}</div>` : '';
        const description = profile.business_description ? `<div style="color:#94a3b8;font-size:12px;margin-top:6px;line-height:1.5;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;">${safeText(profile.business_description)}</div>` : '';
        return `
          <div style="background:#0d1929;border:1px solid #1e2d42;border-radius:16px;margin-bottom:14px;overflow:hidden;text-align:left;">

            <!-- Vendor header -->
            <div style="background:linear-gradient(135deg,#111f35,#0f1722);padding:14px 14px 12px;border-bottom:1px solid #1e2d42;">
              <div style="display:flex;align-items:center;gap:12px;">
                <div style="width:46px;height:46px;border-radius:12px;background:linear-gradient(135deg,#D97706,#92400E);display:flex;align-items:center;justify-content:center;font-weight:900;font-size:16px;color:white;flex-shrink:0;letter-spacing:-0.5px;">${initials}</div>
                <div style="flex:1;min-width:0;">
                  <div style="font-weight:800;font-size:16px;color:#f1f5f9;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${safeText(vendorName)}</div>
                  <div style="font-size:11px;color:#64748b;margin-top:1px;">📍 ${safeText(location)}</div>
                </div>
                <div style="font-size:9px;color:#fbbf24;letter-spacing:0.1em;font-weight:800;background:rgba(251,191,36,0.1);padding:3px 7px;border-radius:20px;border:1px solid rgba(251,191,36,0.2);flex-shrink:0;">VENDOR</div>
              </div>
              <div style="margin-top:10px;display:flex;flex-wrap:wrap;">${catChips}${installBadge}</div>
              ${brands}
              ${description}
            </div>
            ...
            <div id="vendorMarketplaceMore_${profileId}" style="display:none;">
              <div style="padding:12px 14px 4px;">
                <div style="font-size:11px;font-weight:800;color:#fbbf24;letter-spacing:0.06em;margin-bottom:8px;">🛒 PRODUCTS</div>
                <div id="vendorProductsInline_${profileId}"></div>
              </div>
              <div style="padding:4px 14px 12px;border-top:1px solid #1e2d42;margin-top:8px;">
                <div style="font-size:11px;color:#64748b;margin-bottom:4px;line-height:1.5;">
            ...
            <!-- Action row -->
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:12px 14px;background:#0a1628;border-top:1px solid #1e2d42;">
              <button onclick="toggleMarketplaceProfileDetails('vendor','${profileId}')"
                style="padding:11px;background:transparent;border:1.5px solid #334155;border-radius:10px;color:#94a3b8;font-size:13px;font-weight:700;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:5px;">
                🛒 View Products
              </button>
              <button onclick="selectVendorForOffer('${safeText(profile.id)}')"
                style="padding:11px;background:linear-gradient(135deg,#F59E0B,#D97706);border:none;border-radius:10px;color:white;font-size:13px;font-weight:800;cursor:pointer;box-shadow:0 2px 10px rgba(245,158,11,0.35);">
                Request Offer
              </button>
            </div>
          </div>`;
    ...
    function egShowProductDetail(productJson, vendorId) {
      ...
      const specRows = specConfig.map(f => {
        ...
        return `<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid #1e2d42;">
          <span style="color:#94a3b8;font-size:13px;">${f.label}</span>
          <span style="color:#f1f5f9;font-weight:700;font-size:13px;">${safeText(val)}${unit}</span>
        </div>`;
      }).filter(Boolean);

      if (p.brand) specRows.unshift(`<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid #1e2d42;">
        <span style="color:#94a3b8;font-size:13px;">Brand</span>
        <span style="color:#f1f5f9;font-weight:700;font-size:13px;">${safeText(p.brand)}</span>
      </div>`);

      if (p.type) specRows.unshift(`<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid #1e2d42;">
        <span style="color:#94a3b8;font-size:13px;">Category</span>
        <span style="color:#f1f5f9;font-weight:700;font-size:13px;">${safeText(typeLabels[p.type] || p.type)}</span>
      </div>`);

      const qtyDisplay = (p.quantity != null && p.quantity > 0)
        ? `<span style="display:inline-block;padding:3px 10px;background:rgba(34,197,94,0.12);color:#22c55e;font-size:11px;font-weight:700;border-radius:20px;border:1px solid rgba(34,197,94,0.25);">✓ ${p.quantity} in stock</span>`
        : (p.quantity === 0 ? `<span style="display:inline-block;padding:3px 10px;background:rgba(239,68,68,0.12);color:#ef4444;font-size:11px;font-weight:700;border-radius:20px;border:1px solid rgba(239,68,68,0.25);">Out of stock</span>` : '');
      ...
      overlay.innerHTML = `
        <style>
          ...
          #egProductDetailSheet .sheet-inner { animation:egSlideUp 0.28s cubic-bezier(0.34,1.2,0.64,1); }
        </style>
        <div class="sheet-inner" style="
          background:#0d1929;border-radius:24px 24px 0 0;
          width:100%;max-width:520px;max-height:92vh;
          display:flex;flex-direction:column;overflow:hidden;
          border:1px solid #1e2d42;border-bottom:none;
          box-shadow:0 -8px 40px rgba(0,0,0,0.6);
        ">
          <!-- Drag handle -->
          <div style="display:flex;justify-content:center;padding:12px 0 4px;">
            <div style="width:40px;height:4px;background:#334155;border-radius:99px;"></div>
          </div>
          ...
          <div style="overflow-y:auto;flex:1;-webkit-overflow-scrolling:touch;">
            ...
            <!-- Hero image -->
            ${p.image
              ? `<div style="width:100%;height:220px;overflow:hidden;background:#0a1628;">
                   <img src="${safeText(p.image)}" alt="${safeText(p.name||'')}"
                     style="width:100%;height:100%;object-fit:cover;display:block;">
                 </div>`
              : `<div style="width:100%;height:160px;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#111f35,#0a1628);font-size:72px;">
                   ${typeIcons[p.type] || '📦'}
                 </div>`
            }
            ...
            <div style="padding:20px 18px 8px;">
              ...
              <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:6px;">
                <div style="flex:1;">
                  <h2 style="font-size:20px;font-weight:800;color:#f1f5f9;margin:0 0 4px;">${safeText(p.name || 'Product')}</h2>
                  ${p.brand ? `<div style="font-size:13px;color:#94a3b8;">by ${safeText(p.brand)}</div>` : ''}
                </div>
                <button onclick="document.getElementById('egProductDetailSheet').remove()"
                  style="width:32px;height:32px;border-radius:50%;background:#1e2d42;border:none;color:#94a3b8;font-size:18px;cursor:pointer;flex-shrink:0;display:flex;align-items:center;justify-content:center;line-height:1;">×</button>
              </div>

              <!-- Price + stock -->
              <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;flex-wrap:wrap;">
                <span style="font-size:26px;font-weight:900;color:#fbbf24;">₦${Number(p.price||0).toLocaleString()}</span>
                ${qtyDisplay}
              </div>
              ...
              <div style="margin-bottom:16px;">
                <div style="font-size:11px;font-weight:800;color:#64748b;letter-spacing:0.1em;margin-bottom:4px;">SPECIFICATIONS</div>
                <div style="background:#0a1628;border:1px solid #1e2d42;border-radius:12px;padding:0 12px;">
                  ${specRows.join('')}
                </div>
              </div>` : ''}
              ...
              <div style="margin-bottom:20px;">
                <div style="font-size:11px;font-weight:800;color:#64748b;letter-spacing:0.1em;margin-bottom:8px;">DESCRIPTION</div>
                <p style="color:#94a3b8;font-size:14px;line-height:1.7;margin:0;">${safeText(p.description)}</p>
              </div>` : ''}
          ...
          <!-- Fixed CTA -->
          <div style="padding:12px 18px 24px;background:#0d1929;border-top:1px solid #1e2d42;flex-shrink:0;">
            <button onclick="
    ...
    function egMarketplaceBack(type) {
      ...
      // User portal or unknown — go to dashboard if logged in, calculator if not
    ...
    function updateUserAccountUI() {
      ...
      if (bar) bar.style.display = 'none'; // kept hidden; IDs inside still referenced by old code
      ...
      if (isSignedInEndUser()) {
        ...
        if (calcBackBar)  calcBackBar.style.display   = 'block';
        if (guestBackBtn) guestBackBtn.style.display  = 'none';
      } else {
        if (guestBanner)  guestBanner.style.display  = 'block';
        if (calcBackBar)  calcBackBar.style.display   = 'none';
        if (guestBackBtn) guestBackBtn.style.display  = 'block';
      }
    }
    ...
    function egShowUserDashboard() {
      ...
      if (greetEl && currentUser) {
        const name = currentUser.full_name || currentUser.company_name || currentUser.business_name || currentUser.email || 'there';
        greetEl.textContent = name.split(' ')[0]; // first name only
      }
    ...
    function downloadSystemPDF() {
      ...
      const lines = [
        ...
        '\u2022 Battery: ' + kWh + 'kWh LiFePO4 Pack' + (r.batUnits > 1 ? ' (' + r.batUnits + ' units)' : ''),
...
async function supabaseSubmitMarketplaceLead(lead) {
  try {
      ...
      supabaseResetPassword(email).then((res) => {
        ...
        if (res.success) {
          ...
          if (s1) s1.innerHTML = `
            ...
            <button class="btn btn-outline" onclick="openUserAuthModal('signin')" style="width:100%;">Back to Sign In</button>
          `;
        } else {
    ...
    async function userForgotStep3() {
      ...
      // Open modal in sign-in mode so they can log in immediately
      currentRole = 'user';
      showScreen('user-calculator');
      setTimeout(() => openUserAuthModal('signin'), 400);
    }
