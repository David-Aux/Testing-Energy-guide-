// EG-VERSION: v304-fix
    ...
    function populateAllStateSelects(){
      const stateSelects = [
        ...
        "agentState","ugState","ciQuoteState"
      ];
      stateSelects.forEach(id => populateStateSelect(id));
    }

    // ── Document upload clear button helper (Issue 9) ─────────────
    // Adds a filename label + X button after a file is selected.
    window.egShowDocSelected = function (inputId, statusId) {
      const input  = document.getElementById(inputId);
      const status = document.getElementById(statusId);
      if (!input || !status) return;
      if (!input.files || !input.files[0]) { status.innerHTML = ''; return; }
      const name = input.files[0].name;
      status.innerHTML =
        '<span style="color:#22c55e;">📎 ' + escapeHtml(name) + '</span>' +
        '<button onclick="egClearDocUpload(\'' + inputId + '\',\'' + statusId + '\')" ' +
        'style="background:transparent;border:none;color:#ef4444;font-size:16px;font-weight:700;cursor:pointer;padding:0 4px;line-height:1;" ' +
        'title="Remove">✕</button>';
    };
    window.egClearDocUpload = function (inputId, statusId) {
      const input  = document.getElementById(inputId);
      const status = document.getElementById(statusId);
    ...
    async function routeToPortalWithCheck(role) {
      ...
      if (role === 'installer') {
        ...
        if (vendStatus === 'rejected') {
          ...
          // Show incomplete profile banner if core fields are missing
          const _incomplBanner = document.getElementById('vendorIncompleteProfileBanner');
          if (_incomplBanner) {
            _incomplBanner.style.display = isVendorProfileComplete(currentUser) ? 'none' : 'block';
          }
        }
      } else {
    ...
    async function saveCompleteInstallerProfile() {
      ...
      try {
        ...
        // Build payload — only include cac_document_url if we actually got one
        const payload = {
          id: currentUser.id,
          ...
          ...(cac_document_url ? { cac_document_url } : {}),
    ...
    async function saveCompleteVendorProfile() {
      ...
      try {
        ...
        showToast('Profile saved! Your application is under review.', 'success');
        showScreen('vendor-dashboard');
        // Show pending banner — this is always a first-time profile completion
        const _vBanner = document.getElementById('vendorPendingBanner');
        if (_vBanner) _vBanner.style.display = 'block';
      } catch(e) {
        showLoading(false);
        showToast('Failed to save profile: ' + e.message, 'error');
      }
    ...
    async function saveInstallerProfile() {
      ...
      try {
        ...
        showToast('We could not save your profile right now. Please try again.', 'error');
      }
    }
    ...
    async function saveVendorProfile() {
      ...
      try {
        ...
        // Hide incomplete profile banner now that profile is saved
        const _incomplBanner = document.getElementById('vendorIncompleteProfileBanner');
        if (_incomplBanner) _incomplBanner.style.display = 'none';
        const statusEl = document.getElementById('vendorProfileStatus');
        if (statusEl) { statusEl.textContent = '✅ Profile saved successfully!'; statusEl.className = 'success'; statusEl.style.display = 'block'; }
        showToast('Profile updated!', 'success');
    ...
    // ── Sync currentUser for admin checks elsewhere ──────────
    function egSyncUser(user) {
      currentUser = user;
      window.egCurrentUser = user; // expose for modules outside this IIFE (eg-post-calc-extras.js)
      if (typeof egSyncCurrentUser  === 'function') egSyncCurrentUser(user);
      if (typeof egRenderAdminBadge     === 'function') egRenderAdminBadge();
      // Admin panel accessed via URL only — never shown in menu
    ...
    function getMarketplaceUserState() {
      // Prefer the logged-in user's own profile state (ANOMALY-05)
      if (currentUser && currentUser.state) return currentUser.state;
    ...
    async function egLoadVendorProductsInline(vendorId) {
      ...
      try {
        ...
        if (error) {
          // Don't mark as loaded on a real query error (e.g. permission/RLS issue) —
          // log it so it's visible in devtools, and allow a retry on next open.
          console.error('egLoadVendorProductsInline: failed to load vendor_products for', vendorId, error);
          container.innerHTML = '<div style="font-size:12px;color:#ef4444;font-style:italic;padding:8px 0;">Couldn\'t load products. Tap to retry.</div>';
          container.onclick = () => { delete container.dataset.loaded; container.onclick = null; egLoadVendorProductsInline(vendorId); };
          return;
        }
        ...
        return `
          <div style="background:#0f1722;border:1px solid #1e2d42;border-radius:12px;padding:16px;margin-bottom:12px;text-align:left;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:12px;">
              ...
              ${window._ciContext === 'user' && window.l4ci__lastResult
                ? `<button onclick="ciOpenQuoteForm('installer')"
                    style="padding:10px;background:linear-gradient(135deg,#0369a1,#0ea5e9);border:none;border-radius:8px;color:white;font-size:13px;font-weight:700;cursor:pointer;">
                    🏭 C&I Quote
                   </button>`
        ...
        return `
          <div style="background:#0d1929;border:1px solid #1a2740;border-radius:18px;margin-bottom:16px;overflow:hidden;text-align:left;box-shadow:0 4px 24px rgba(0,0,0,0.3);">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:14px 16px;">
              ...
              ${window._ciContext === 'user' && window.l4ci__lastResult
                ? `<button onclick="ciOpenQuoteForm('vendor')"
                    style="padding:12px;background:linear-gradient(135deg,#0369a1,#0ea5e9);border:none;border-radius:12px;color:white;font-size:14px;font-weight:700;cursor:pointer;box-shadow:0 3px 14px rgba(14,165,233,0.3);letter-spacing:0.01em;">
                    🏭 C&I Quote
                   </button>`
    ...
    function egShowProductDetail(productJson, vendorId) {
      ...
      // True when a vendor is previewing their own listed product (not a consumer browsing)
      const ownPreview = !!(currentUser && vendorId && currentUser.id === vendorId);
      ...
      overlay.innerHTML = `
        ...
        ">
          ...
          <div style="padding:14px 18px 28px;background:linear-gradient(to top,#0b1525 80%,transparent);border-top:1px solid #1a2740;flex-shrink:0;">
            ${ownPreview
              ? `<button onclick="document.getElementById('egProductDetailSheet').remove()" style="
                  width:100%;padding:17px;
                  background:#1a2740;
                  border:1px solid #334155;border-radius:14px;
                  color:#94a3b8;font-size:16px;font-weight:700;
                  cursor:pointer;letter-spacing:0.02em;">✕ Close Preview</button>`
              : `<button onclick="
                ...
                " style="
                  ...
                  letter-spacing:0.02em;">⚡ Get Offer for this Product</button>`
            }
          </div>
        </div>
      `;
    ...
    let _egAdminView = 'applications'; // 'applications'|'users'|'corporate'|'agents'|'leads'|'dropoffs'
    let _egAdminSearch = '';
    let _egAdminSearchTimer = null;

    function egAdminSetView(view) {
      _egAdminView = view;
      _egAdminSearch = '';
      const searchInput = document.getElementById('egAdminSearchInput');
      if (searchInput) searchInput.value = '';

      const views = ['applications','users','corporate','agents','leads','dropoffs'];
      views.forEach(v => {
        const btn = document.getElementById('egAdminView' + v.charAt(0).toUpperCase() + v.slice(1));
        if (!btn) return;
        const active = v === view;
        btn.style.background = active ? '#6366f1' : 'transparent';
        btn.style.color      = active ? '#fff' : '#94a3b8';
        btn.style.border     = active ? 'none' : '2px solid #1e293b';
      });

      const appControls = document.getElementById('egAdminApplicationsControls');
      const searchBox    = document.getElementById('egAdminSearchBox');
      if (appControls) appControls.style.display = (view === 'applications') ? '' : 'none';
      if (searchBox)    searchBox.style.display   = (view === 'applications') ? 'none' : '';

      if (view === 'applications')   egLoadAdminPanel();
      else if (view === 'users')     egAdminLoadUsers();
      else if (view === 'corporate') egAdminLoadCorporate();
      else if (view === 'agents')    egAdminLoadAgents();
      else if (view === 'leads')     egAdminLoadLeads();
      else if (view === 'dropoffs')  egAdminLoadDropoffs();
    }

    function egAdminSearchDebounced() {
      const input = document.getElementById('egAdminSearchInput');
      _egAdminSearch = (input?.value || '').trim();
      clearTimeout(_egAdminSearchTimer);
      _egAdminSearchTimer = setTimeout(() => egAdminSetView(_egAdminView), 350);
    }

    function egAdminEmptyState(el, msg) {
      el.innerHTML = '<div class="info" style="text-align:center;padding:24px 12px;">' + escapeHtml(msg) + '</div>';
    }
    function egAdminErrorState(el, err) {
      el.innerHTML = '<div class="info" style="color:#ef4444;text-align:center;padding:24px 12px;">⚠️ ' + escapeHtml((err && err.message) || 'Could not load data') + '</div>';
    }
    function egAdminDate(d) {
      return d ? new Date(d).toLocaleDateString('en-NG', { day:'numeric', month:'short', year:'numeric', hour:'2-digit', minute:'2-digit' }) : '—';
    }

    // ── Users view ──────────────────────────────────────────────────
    async function egAdminLoadUsers() {
      const el = document.getElementById('egAdminPanelList');
      if (!el || !egIsAdmin()) return;
      el.innerHTML = '<div class="info">Loading users...</div>';
      try {
        let q = supabaseClient.from('profiles')
          .select('id, full_name, email, phone, whatsapp_number, state, city, role, is_user, created_at, last_login_at')
          .or('role.eq.user,is_user.eq.true')
          .order('created_at', { ascending: false })
          .limit(100);
        if (_egAdminSearch) {
          const s = _egAdminSearch.replace(/[%,]/g, '');
          q = q.or(`full_name.ilike.%${s}%,email.ilike.%${s}%,phone.ilike.%${s}%`);
        }
        const { data, error } = await q;
        if (error) { egAdminErrorState(el, error); return; }
        if (!data || !data.length) { egAdminEmptyState(el, 'No registered users found.'); return; }

        // Batch-check which of these users have ever touched the C&I calculator
        let ciUserIds = new Set();
        try {
          const ids = data.map(u => u.id).filter(Boolean);
          if (ids.length) {
            const { data: ciRows } = await supabaseClient.from('calculation_events')
              .select('user_id').like('portal', 'ci-%').in('user_id', ids);
            (ciRows || []).forEach(r => ciUserIds.add(r.user_id));
          }
        } catch (e) { /* badge is a nice-to-have, never block the list on it */ }

        el.innerHTML = '<div style="font-size:12px;color:#64748b;margin-bottom:10px;">' + data.length + ' user(s)</div>' +
          data.map(u => `
            <div style="background:#0f172a;border:1px solid #1e293b;border-radius:12px;padding:14px;margin-bottom:10px;">
              <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:6px;">
                <div style="font-weight:700;color:#f1f5f9;font-size:14px;">${escapeHtml(u.full_name || 'Unnamed')}</div>
                ${ciUserIds.has(u.id) ? '<span style="font-size:10px;font-weight:700;padding:3px 8px;border-radius:999px;background:rgba(14,165,233,0.15);color:#38bdf8;flex-shrink:0;">🏭 C&amp;I</span>' : ''}
              </div>
              <div style="font-size:12px;color:#94a3b8;margin-top:4px;">📧 ${escapeHtml(u.email || '—')}</div>
              <div style="font-size:12px;color:#94a3b8;">📱 ${escapeHtml(u.phone || u.whatsapp_number || '—')} &nbsp;·&nbsp; 📍 ${escapeHtml([u.city, u.state].filter(Boolean).join(', ') || '—')}</div>
              <div style="font-size:11px;color:#64748b;margin-top:6px;">Joined ${egAdminDate(u.created_at)} · Last login ${egAdminDate(u.last_login_at)}</div>
            </div>`).join('');
      } catch (err) { egAdminErrorState(el, err); }
    }

    // ── Corporate Partners view ────────────────────────────────────
    async function egAdminLoadCorporate() {
      const el = document.getElementById('egAdminPanelList');
      if (!el || !egIsAdmin()) return;
      el.innerHTML = '<div class="info">Loading corporate partners...</div>';
      try {
        let q = supabaseClient.from('corporate_partners').select('*').order('created_at', { ascending: false }).limit(100);
        if (_egAdminSearch) {
          const s = _egAdminSearch.replace(/[%,]/g, '');
          q = q.or(`company_name.ilike.%${s}%,contact_name.ilike.%${s}%,email.ilike.%${s}%,company_code.ilike.%${s}%`);
        }
        const { data, error } = await q;
        if (error) { egAdminErrorState(el, error); return; }
        if (!data || !data.length) { egAdminEmptyState(el, 'No corporate partners yet.'); return; }
        el.innerHTML = '<div style="font-size:12px;color:#64748b;margin-bottom:10px;">' + data.length + ' compan' + (data.length===1?'y':'ies') + '</div>' +
          data.map(c => `
            <div style="background:#0f172a;border:1px solid #1e293b;border-radius:12px;padding:14px;margin-bottom:10px;">
              <div style="font-weight:700;color:#f1f5f9;font-size:14px;">🏢 ${escapeHtml(c.company_name || 'Unnamed')} <span style="font-size:11px;color:#a78bfa;font-weight:600;">#${escapeHtml(c.company_code || '')}</span></div>
              <div style="font-size:12px;color:#94a3b8;margin-top:4px;">Contact: ${escapeHtml(c.contact_name || '—')} · ${escapeHtml(c.industry || '—')}</div>
              <div style="font-size:12px;color:#94a3b8;">📧 ${escapeHtml(c.email || '—')} &nbsp;·&nbsp; 📱 ${escapeHtml(c.phone || '—')}</div>
              <div style="font-size:11px;color:#64748b;margin-top:6px;">Joined ${egAdminDate(c.created_at)}</div>
            </div>`).join('');
      } catch (err) { egAdminErrorState(el, err); }
    }

    // ── Agents / Network Submissions view ──────────────────────────
    async function egAdminLoadAgents() {
      const el = document.getElementById('egAdminPanelList');
      if (!el || !egIsAdmin()) return;
      el.innerHTML = '<div class="info">Loading agent submissions...</div>';
      try {
        let q = supabaseClient.from('agent_submissions').select('*').order('created_at', { ascending: false }).limit(100);
        if (_egAdminSearch) {
          const s = _egAdminSearch.replace(/[%,]/g, '');
          q = q.or(`agent_name.ilike.%${s}%,agent_code.ilike.%${s}%,company_code.ilike.%${s}%`);
        }
        const { data, error } = await q;
        if (error) { egAdminErrorState(el, error); return; }
        if (!data || !data.length) { egAdminEmptyState(el, 'No agent submissions yet.'); return; }
        el.innerHTML = '<div style="font-size:12px;color:#64748b;margin-bottom:10px;">' + data.length + ' submission(s)</div>' +
          data.map(a => `
            <div style="background:#0f172a;border:1px solid #1e293b;border-radius:12px;padding:14px;margin-bottom:10px;">
              <div style="display:flex;justify-content:space-between;align-items:flex-start;">
                <div style="font-weight:700;color:#f1f5f9;font-size:14px;">${escapeHtml(a.agent_name || 'Agent')} <span style="font-size:11px;color:#64748b;font-weight:400;">#${escapeHtml(a.agent_code || '')}</span></div>
                <span style="font-size:10px;font-weight:700;padding:3px 8px;border-radius:999px;background:${a.system_type === 'ci' ? 'rgba(14,165,233,0.15);color:#38bdf8' : 'rgba(34,197,94,0.15);color:#22c55e'};">${a.system_type === 'ci' ? '🏭 C&I' : '🏠 Residential'}</span>
              </div>
              <div style="font-size:11px;color:#a78bfa;margin-top:2px;">Company: #${escapeHtml(a.company_code || '—')} · ${escapeHtml(a.agent_type || 'agent')}</div>
              <div style="font-size:12px;color:#94a3b8;margin-top:6px;background:#0a0e17;border-radius:8px;padding:8px;font-family:monospace;">${escapeHtml(a.sizing_summary || '—')}</div>
              <div style="font-size:12px;color:#94a3b8;margin-top:6px;">📱 ${escapeHtml(a.phone || '—')} &nbsp;·&nbsp; 📍 ${escapeHtml([a.city, a.state].filter(Boolean).join(', ') || '—')}</div>
              <div style="font-size:11px;color:#64748b;margin-top:6px;">${egAdminDate(a.created_at)}</div>
            </div>`).join('');
      } catch (err) { egAdminErrorState(el, err); }
    }

    // ── Marketplace Leads view ─────────────────────────────────────
    async function egAdminLoadLeads() {
      const el = document.getElementById('egAdminPanelList');
      if (!el || !egIsAdmin()) return;
      el.innerHTML = '<div class="info">Loading leads...</div>';
      try {
        let q = supabaseClient.from('marketplace_leads').select('*').order('created_at', { ascending: false }).limit(100);
        if (_egAdminSearch) {
          const s = _egAdminSearch.replace(/[%,]/g, '');
          q = q.or(`full_name.ilike.%${s}%,email.ilike.%${s}%,phone.ilike.%${s}%`);
        }
        const { data, error } = await q;
        if (error) { egAdminErrorState(el, error); return; }
        if (!data || !data.length) { egAdminEmptyState(el, 'No leads yet.'); return; }
        const statusColor = { open:'#f59e0b', vendor_requested:'#f59e0b', installer_requested:'#f59e0b', installer_contacted:'#38bdf8', closed:'#22c55e' };
        el.innerHTML = '<div style="font-size:12px;color:#64748b;margin-bottom:10px;">' + data.length + ' lead(s)</div>' +
          data.map(l => {
            const isCI = !!(l.note && l.note.indexOf('[C&I') === 0);
            return `
            <div style="background:#0f172a;border:1px solid ${isCI ? 'rgba(14,165,233,0.35)' : '#1e293b'};border-radius:12px;padding:14px;margin-bottom:10px;">
              <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:6px;">
                <div style="font-weight:700;color:#f1f5f9;font-size:14px;">${escapeHtml(l.full_name || 'Unnamed')}</div>
                <div style="display:flex;gap:6px;flex-shrink:0;">
                  ${isCI ? '<span style="font-size:10px;font-weight:700;padding:3px 8px;border-radius:999px;background:rgba(14,165,233,0.15);color:#38bdf8;">🏭 C&amp;I</span>' : ''}
                  <span style="font-size:10px;font-weight:700;padding:3px 8px;border-radius:999px;background:${(statusColor[l.status]||'#64748b')}22;color:${statusColor[l.status]||'#94a3b8'};">${escapeHtml(l.status || '—')}</span>
                </div>
              </div>
              <div style="font-size:12px;color:#94a3b8;margin-top:4px;">${escapeHtml(l.project_type || '—')} · ${escapeHtml([l.city, l.state].filter(Boolean).join(', ') || '—')}</div>
              <div style="font-size:12px;color:#94a3b8;">📧 ${escapeHtml(l.email || '—')} &nbsp;·&nbsp; 📱 ${escapeHtml(l.phone || '—')}</div>
              <div style="font-size:11px;color:#64748b;margin-top:6px;">${egAdminDate(l.created_at)}</div>
            </div>`;
          }).join('');
      } catch (err) { egAdminErrorState(el, err); }
    }

    // ── Drop-offs view — calculated but never reached out ──────────
    // Two sources, shown together:
    //  1. user_gate_registrations — the AUTHORITATIVE source for the User
    //     portal. Registration is required before calculating, so every
    //     row has guaranteed contact info. calc_completed=true and
    //     lead_submitted=false means "calculated, never requested a quote."
    //  2. calculation_events — covers Installer/Vendor/C&I, where contact
    //     info is only present if someone voluntarily used the optional
    //     "send me this" capture box.
    async function egAdminLoadDropoffs() {
      const el = document.getElementById('egAdminPanelList');
      if (!el || !egIsAdmin()) return;
      el.innerHTML = '<div class="info">Loading drop-offs...</div>';
      try {
        let gateQ = supabaseClient.from('user_gate_registrations')
          .select('*')
          .eq('calc_completed', true)
          .eq('lead_submitted', false)
          .order('registered_at', { ascending: false })
          .limit(100);
        let calcQ = supabaseClient.from('calculation_events').select('*')
        ...
        if (_egAdminSearch) {
          const s = _egAdminSearch.replace(/[%,]/g, '');
          gateQ = gateQ.or(`full_name.ilike.%${s}%,email.ilike.%${s}%,phone.ilike.%${s}%`);
          calcQ = calcQ.or(`contact_email.ilike.%${s}%,contact_phone.ilike.%${s}%`);
        }
        const [gateRes, calcRes] = await Promise.all([gateQ, calcQ]);
        if (gateRes.error && calcRes.error) { egAdminErrorState(el, gateRes.error); return; }

        const gateRows = gateRes.data || [];
        const calcRows = calcRes.data || [];

        if (!gateRows.length && !calcRows.length) {
          egAdminEmptyState(el, '✅ No outstanding follow-ups — everyone who calculated and left contact info either converted or hasn\'t calculated recently.');
          return;
        }
        ...
        if (gateRows.length) {
          html += '<div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:#38bdf8;margin:4px 0 8px;">🏠 User Portal — ' + gateRows.length + ' calculated, never requested a quote</div>';
          html += gateRows.map(g => `
            <div style="background:#0f172a;border:1px solid rgba(56,189,248,0.3);border-radius:12px;padding:14px;margin-bottom:10px;">
              <div style="display:flex;justify-content:space-between;align-items:flex-start;">
                <div style="font-weight:700;color:#f1f5f9;font-size:14px;">${escapeHtml(g.full_name || 'Unnamed')}</div>
                <span style="font-size:11px;color:#64748b;">${egAdminDate(g.registered_at)}</span>
              </div>
              <div style="font-size:13px;color:#f1f5f9;margin-top:8px;">📧 ${escapeHtml(g.email || '—')} &nbsp;·&nbsp; 📱 ${escapeHtml(g.phone || '—')} &nbsp;·&nbsp; 📍 ${escapeHtml(g.state || '—')}</div>
              <button onclick="egAdminMarkGateFollowedUp('${escapeHtml(g.email)}')" style="width:100%;margin-top:10px;padding:9px;background:transparent;border:1px solid #22c55e;border-radius:8px;color:#22c55e;font-size:12px;font-weight:700;cursor:pointer;">✓ Mark as Followed Up</button>
            </div>`).join('');
        }

        if (calcRows.length) {
          html += '<div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:#a78bfa;margin:18px 0 8px;">🛠 Installer / Vendor / C&amp;I — ' + calcRows.length + ' calculation(s)</div>';
          html += calcRows.map(d => `
            <div style="background:#0f172a;border:1px solid rgba(245,158,11,0.3);border-radius:12px;padding:14px;margin-bottom:10px;">
              <div style="display:flex;justify-content:space-between;align-items:flex-start;">
                <span style="font-size:10px;font-weight:700;padding:3px 8px;border-radius:999px;background:rgba(99,102,241,0.15);color:#a5b4fc;">${escapeHtml(d.portal || 'unknown')}</span>
                <span style="font-size:11px;color:#64748b;">${egAdminDate(d.created_at)}</span>
              </div>
              <div style="font-size:12px;color:#cbd5e1;margin-top:8px;background:#0a0e17;border-radius:8px;padding:8px;font-family:monospace;">${escapeHtml(d.sizing_summary || 'No sizing summary')}</div>
              <div style="font-size:13px;color:#f1f5f9;margin-top:8px;">${d.contact_email ? '📧 ' + escapeHtml(d.contact_email) : ''}${d.contact_email && d.contact_phone ? ' &nbsp;·&nbsp; ' : ''}${d.contact_phone ? '📱 ' + escapeHtml(d.contact_phone) : ''}</div>
              <button onclick="egAdminMarkFollowedUp('${escapeHtml(d.id)}')" style="width:100%;margin-top:10px;padding:9px;background:transparent;border:1px solid #22c55e;border-radius:8px;color:#22c55e;font-size:12px;font-weight:700;cursor:pointer;">✓ Mark as Followed Up</button>
            </div>`).join('');
        }
    ...
    async function egAdminMarkGateFollowedUp(email) {
      if (!egIsAdmin() || !email) return;
      try {
        await supabaseClient.from('user_gate_registrations').update({ lead_submitted: true }).eq('email', email);
    ...
    async function egAdminMarkFollowedUp(eventId) {
      if (!egIsAdmin()) return;
      try {
        await supabaseClient.from('calculation_events').update({ converted_to_lead: true }).eq('id', eventId);
        showToast('Marked as followed up', 'success');
        egAdminLoadDropoffs();
      } catch (err) {
        showToast('Could not update — try again', 'error');
    ...
    async function egLoadAdminPanel() {
      ...
      // Load stats (pending/approved/rejected counts)
      try {
        const statQueries = await Promise.all([
          supabaseClient.from('profiles').select('id', {count:'exact',head:true}).eq('status','pending'),
          supabaseClient.from('profiles').select('id', {count:'exact',head:true}).eq('status','approved'),
          supabaseClient.from('profiles').select('id', {count:'exact',head:true}).eq('status','rejected'),
        ]);
        const [pending, approved, rejected] = statQueries;
        const setEl = (id, val) => { const e = document.getElementById(id); if (e) e.textContent = val ?? '—'; };
        setEl('egAdminStatPending',  pending.count  ?? 0);
        setEl('egAdminStatApproved', approved.count ?? 0);
        setEl('egAdminStatRejected', rejected.count ?? 0);
    ...
    async function egLoadLeadProductCheckboxes(vendorId) {
      ...
      try {
        ...
        if (error) {
          console.error('egLoadLeadProductCheckboxes: failed to load vendor_products for', vendorId, error);
        }
    ...
    // Dynamic what-happens-next message on lead success screen (ANOMALY-16)
    function updateLeadSuccessNextSteps(state, contactMethod, email) {
      const el = document.getElementById('leadSuccessNextSteps');
      if (!el) return;
      const methodLabels = { phone: 'a phone call', whatsapp: 'a WhatsApp message', email: 'an email' };
      const methodText = methodLabels[contactMethod] || 'a phone call or WhatsApp message';
      const stateText  = state ? ' in ' + state : '';
      const emailLine  = email
        ? ' A summary has also been sent to <strong style="color:#f3f4f6;">' + escapeHtml(email) + '</strong>.'
        : '';
      el.innerHTML =
        '<strong style="color:#f3f4f6;">What happens next?</strong><br/>' +
        'A verified installer' + stateText + ' will reach out via ' + methodText + ' within 24–48 hours. ' +
        'You don’t need to submit again.' + emailLine;
    }

    // Live character counter for lead note textarea (ANOMALY-14)
    function updateLeadNoteCount(el) {
      const counter = document.getElementById('leadNoteCount');
      if (!counter) return;
      const max = el.getAttribute('maxlength') || 500;
      counter.textContent = el.value.length + ' / ' + max;
      counter.style.color = el.value.length >= max ? '#ef4444' : 'var(--eg-muted)';
    }
    ...
    async function submitLead() {
      ...
      if (isValid) isValid = normalizePhoneField('leadPhone', 'leadPhoneError') && isValid;
      ...
      try {
        ...
        if (result.success) {
          ...
          updateLeadSuccessNextSteps(state, contact, email);
          ...
          // Data capture: this calculation session converted to a real lead
    ...
    function showToast(message, type = 'success') {
      ...
      closeBtn.style.cssText = 'margin-left:10px;background:none;border:none;color:inherit;font-size:18px;line-height:1;cursor:pointer;padding:0 2px;opacity:0.7;';
      const dismiss = () => {
        toast.style.animation = 'slideIn 0.3s ease-out reverse';
        setTimeout(() => toast.remove(), 300);
      };
      closeBtn.onclick = dismiss;
      toast.innerHTML = '<span style="font-size:20px;">' + (icons[type]||'') + '</span><span style="flex:1;">' + message + '</span>';
      toast.style.cssText = 'display:flex;align-items:center;';
      toast.appendChild(closeBtn);
      document.body.appendChild(toast);
      // All toasts auto-dismiss — errors after 6s, others after 4s
      const dismissDelay = (type === 'error' || type === 'warning') ? 6000 : 4000;
      setTimeout(dismiss, dismissDelay);
    }
    
    function validateNigerianPhone(raw) {
      const digits = (raw || '').replace(/[\s\-()]/g, '');
      var m = digits.match(/^(\+?234)(\d{10})$/);
      if (m) return { valid: true, normalized: '+234' + m[2] };
      m = digits.match(/^0(\d{10})$/);
      if (m) return { valid: true, normalized: '+234' + m[1] };
      return { valid: false, normalized: digits };
    }
    function normalizePhoneField(fieldId, errorId) {
      const field = document.getElementById(fieldId);
      const errorDiv = errorId ? document.getElementById(errorId) : null;
      if (!field || !field.value.trim()) return true;
      const result = validateNigerianPhone(field.value);
      if (!result.valid) {
        field.classList.add('input-error');
        if (errorDiv) errorDiv.textContent = 'Enter a valid Nigerian number, e.g. 080XXXXXXXX';
        return false;
      }
    ...
    function showLoading(show = true, text = 'Processing...') {
      ...
      if (window._egLoadingTimeout) { clearTimeout(window._egLoadingTimeout); window._egLoadingTimeout = null; }
      if (show) {
        window._egLoadingTimeout = setTimeout(function() {
          document.getElementById('loadingOverlay').classList.toggle('active', false);
          showToast('This is taking longer than usual. Check your connection and try again.', 'warning');
          window._egLoadingTimeout = null;
        }, 15000);
      }
    }
    ...
    // ── C&I Handoff ────────────────────────────────────────────────────────────
    // Called from oversize banners in other calculators.
    // v2 CHANGE: no longer stashes peakKw/dailyKwh or pre-fills anything —
    // the v2 C&I engine's primary input is monthly_bill/monthly_energy_kwh,
    // which residential peak-kW/daily-kWh data doesn't map cleanly onto.
    // Per decision: drop the pre-fill entirely. The user lands on the C&I
    // form with a short context note (handled by calc-ci.js's
    // eg:ci-handoff listener) and enters their bill/kWh fresh.
    window.egCIHandoff = function(peakKw, dailyKwh) {
      // Infer which portal this handoff came from so the right post-calc
      // actions panel (user/installer/vendor) shows once results render.
      window._ciContext = (currentRole === 'installer' || currentRole === 'vendor') ? currentRole : 'user';
      showScreen('ci-calculator');
      if (typeof ciApplyContextUI === 'function') ciApplyContextUI();
      // Dispatch event so calc-ci.js can show the "commercial-scale load
      // detected" note after screen transition — no data payload needed.
      setTimeout(function(){ document.dispatchEvent(new CustomEvent('eg:ci-handoff')); }, 80);
    };
    ...
    function showScreen(screenId) {
      ...
      if (screenId === 'admin-panel') { egAdminSetTab('pending'); egAdminSetRole('all'); if (egIsAdmin()) egAdminSetView('applications'); }
    ...
    function openUserAuthModal(mode = 'signup', context = 'track_lead') {
      ...
      if (context === 'track_lead') {
        ...
        if (text) text.textContent = 'Sign up to access your dashboard and track your requests.';
      }
    ...
    async function restoreEnergyGuideUserSession(sessionArg = null) {
      // Don't run during/after manual logout, or while a portal-gate check /
      // dual-portal recovery sign-in is deliberately deciding routing itself.
      if (window._egLoggingOut) return;
      if (window._egSuppressAutoRoute) return;
      try {
        ...
        if (profile) {
          window.egHasFullProfile = true; // full portal user — cloud save allowed
          egSyncUser({ ...session.user, ...profile });
          if (!currentRole || currentRole === 'user') currentRole = profile.role || 'user';
          updateUserAccountUI();
        } else {
          window.egHasFullProfile = false; // gate-only user — no profiles row, local save only
          egSyncUser({
            ...session.user,
            id: session.user.id,
    ...
    async function handleUserAuthSubmit() {
      ...
      // Suppress the global SIGNED_IN listener's auto-route-to-portal logic
      // for the duration of this function — we decide routing ourselves
      // below (gate check / dual-portal recovery merge), and don't want a
      // stale Installer/Vendor profile silently hijacking navigation.
      window._egSuppressAutoRoute = true;
      try {
        showLoading(true, userAuthMode === 'signup' ? 'Creating account...' : 'Signing in...');
        let result;
        let recoveredProfile = null; // set when signup adds personal access to an existing professional/corporate account
        ...
        if (userAuthMode === 'signup') {
          ...
          if (!result.success && result.error === 'already_registered') {
            // Email already has an account (Installer/Vendor/Corporate) —
            // add personal/homeowner access to it instead of blocking.
            showLoading(true, 'Adding personal access...');
            try {
              ...
              if (signInErr || !signInData || !signInData.user) {
                showToast('This email already has an account. Enter its existing password above to add personal access, or <a href="javascript:void(0)" onclick="openUserForgotPassword()" style="color:#fff;text-decoration:underline;font-weight:700;">reset your password</a>.', 'error');
                return;
              }
              ...
              const alreadyHasUser = existingProfile && (existingProfile.role === 'user' || existingProfile.is_user === true);
              if (alreadyHasUser) {
                showToast('You already have a personal account. Use Sign In below.', 'info');
                switchUserAuthMode('signin');
                return;
              }
              // Add personal access, preserving any existing Installer/Vendor data untouched.
              await supabaseClient.from('profiles').upsert([{
                id: existingUser.id,
                is_user: true,
                role: (existingProfile && existingProfile.role) ? existingProfile.role : 'user',
                full_name: (existingProfile && existingProfile.full_name) ? existingProfile.full_name : (fullName || null)
              }], { onConflict: 'id' });
              result = { success: true, user: existingUser, emailConfirmationRequired: false };
              recoveredProfile = { ...(existingProfile || {}), is_user: true };
            } catch (addErr) {
              showToast('This email already has an account. Enter its existing password above to add personal access, or <a href="javascript:void(0)" onclick="openUserForgotPassword()" style="color:#fff;text-decoration:underline;font-weight:700;">reset your password</a>.', 'error');
              return;
            }
        ...
        let profile = recoveredProfile || result.profile || null;
        ...
        // ── User Portal eligibility gate (sign-in only) ──────────────────
        // Block accounts that are exclusively Installer/Vendor/Corporate
        // Partner. Plain residential accounts (no profile row, by design)
        // and any account with is_user/role:'user' are always allowed.
        if (userAuthMode === 'signin') {
          const isInstaller = !!profile && (profile.role === 'installer' || profile.is_installer === true);
          const isVendor    = !!profile && (profile.role === 'vendor'    || profile.is_vendor    === true);
          const hasUserAccess = !!profile && (profile.role === 'user' || profile.is_user === true);

          let blocked = false;
          let blockedAs = '';
          if ((isInstaller || isVendor) && !hasUserAccess) {
            blocked = true;
            blockedAs = (isInstaller && isVendor) ? 'an Installer/Vendor' : (isInstaller ? 'an Installer' : 'a Vendor');
          } else if (!hasUserAccess) {
            const { data: corpRow } = await supabaseClient.from('corporate_partners').select('id').eq('user_id', authUser.id).maybeSingle();
            if (corpRow) { blocked = true; blockedAs = 'a Corporate Partner'; }
          }

          if (blocked) {
            await supabaseClient.auth.signOut();
            showToast('This email is registered as ' + blockedAs + ' account. Sign in on that portal, or tap "Create Free Account" below to add personal/homeowner access.', 'error');
            switchUserAuthMode('signup');
            return;
          }
        }

        currentUser = { ...(authUser || {}), ...(profile || {}), role: 'user', full_name: profile?.full_name || fullName || authUser?.user_metadata?.full_name || authUser?.user_metadata?.name || '' };
        currentRole = 'user';
        closeUserAuthModal();
        ...
        if (userAuthMode === 'signin' && authUser && authUser.id) { supabaseClient.from('profiles').update({ last_login_at: new Date().toISOString() }).eq('id', authUser.id).then(()=>{}, ()=>{}); }

        if (pendingUserAuthAction === 'track_lead') {
          egShowUserDashboard();
        } else {
    ...
    // Load any gate-locally-saved calculation (residential or C&I) by sorted index
    // Delete any gate-locally-saved calculation by sorted index
    // Load a cloud-saved user calculation into the calculator (Issue 6 — was undefined)
    // Delete a cloud-saved user calculation (Issue 7 — was undefined)
...
// ================================================================
    ...
    async function renderJobLeads() {
      ...
      try {
        ...
        // Use secure leads_for_portal() function — masks phone/email
        // until this installer has an unlock row. (ANOMALY-23)
        const leadsPromise = supabaseClient.rpc('leads_for_portal', {
          statuses:  ['installer_requested', 'installer_contacted', 'closed', 'open', 'new'],
          state_val: installerState || '',
          portal:    'installer',
          lim:       200
        });
        ...
        if (leads.length === 0) {
          container.innerHTML = installerState
            ? `<div style="text-align:center;padding:40px 20px;"><div style="font-size:36px;margin-bottom:10px;">📭</div><strong>No leads yet in ${installerState}.</strong><p style="font-size:13px;color:#687076;margin-top:6px;max-width:280px;margin-left:auto;margin-right:auto;">New customer requests in your state will show up here automatically. Make sure your profile is complete.</p></div>`
            : `<div style="text-align:center;padding:40px 20px;"><div style="font-size:36px;margin-bottom:10px;">📭</div><strong>No job leads yet.</strong><p style="font-size:13px;color:#687076;margin-top:6px;">Add your state to your profile to enable location matching.</p><button class="btn btn-success" onclick="showScreen('installer-profile')" style="margin-top:14px;max-width:240px;">Update Profile →</button></div>`;
          return;
        }

        // Contact details are always visible — there is no paywall.
        ...
        const phone = (lead.phone && isUnlocked) ? lead.phone : null;
        const email = (lead.email && isUnlocked) ? lead.email : null;
        ...
        if (err.message === 'timeout') {
          container.innerHTML = '<div style="text-align:center;padding:40px 20px;"><div style="font-size:32px;margin-bottom:10px;">⏱️</div><p style="color:#ef4444;margin-bottom:14px;">Loading timed out. Check your connection.</p><button class="btn btn-outline" onclick="renderJobLeads()" style="max-width:160px;margin:0 auto;">↻ Try Again</button></div>';
        } else {
          container.innerHTML = '<div style="text-align:center;padding:40px 20px;"><div style="font-size:32px;margin-bottom:10px;">⚠️</div><p style="color:#ef4444;margin-bottom:14px;">Could not load job leads. Please try again.</p><button class="btn btn-outline" onclick="renderJobLeads()" style="max-width:160px;margin:0 auto;">↻ Try Again</button></div>';
        }
      }
    }
    ...
    async function handleInstallerLogin() {
      ...
      if (result.success && isInstaller) {
        ...
        if (currentUser && currentUser.id) { supabaseClient.from('profiles').update({ last_login_at: new Date().toISOString() }).eq('id', currentUser.id).then(()=>{}, ()=>{}); }
    ...
    async function renderVendorRequests() {
      ...
      const vendorState = await getCurrentUserPortalState('vendor');
      const { data, error } = await supabaseClient.rpc('leads_for_portal', {
        statuses:  ['vendor_requested', 'vendor_responded', 'closed'],
        state_val: vendorState || '',
        portal:    'vendor',
        lim:       200
      });
      ...
      if (error) {
        ...
        container.innerHTML = '<div style="text-align:center;padding:40px 20px;"><div style="font-size:32px;margin-bottom:10px;">⚠️</div><p style="color:#ef4444;margin-bottom:14px;">Could not load vendor requests. Please try again.</p><button class="btn btn-outline" onclick="renderVendorRequests()" style="max-width:160px;margin:0 auto;">↻ Try Again</button></div>';
        return;
      }

      if (!data || data.length === 0) {
        container.innerHTML = vendorState
          ? `<div style="text-align:center;padding:40px 20px;"><div style="font-size:36px;margin-bottom:10px;">📭</div><strong>No requests yet in ${vendorState}.</strong><p style="font-size:13px;color:#687076;margin-top:6px;max-width:280px;margin-left:auto;margin-right:auto;">New product requests from customers in your area will appear here.</p></div>`
          : `<div style="text-align:center;padding:40px 20px;"><div style="font-size:36px;margin-bottom:10px;">📭</div><strong>No vendor requests yet.</strong><button class="btn btn-warning" onclick="showScreen('vendor-profile')" style="margin-top:14px;max-width:240px;">Update Profile →</button></div>`;
        return;
      }
        ...
        container.innerHTML = products.map(product => {
          ...
          <div class="product-card" data-product="${detailJson}">
            <div style="position:relative;flex-shrink:0;">
              ${product.image
                ? `<img src="${product.image}" class="product-image" alt="${product.name}" style="cursor:pointer;" onclick="egShowProductDetail(JSON.parse(this.closest('.product-card').dataset.product), currentUser?.id)" >`
                : `<div class="product-image" style="display:flex;align-items:center;justify-content:center;font-size:32px;background:#fef3c7;cursor:pointer;" onclick="egShowProductDetail(JSON.parse(this.closest('.product-card').dataset.product), currentUser?.id)">${typeIcons[product.type] || '📦'}</div>`
              }
            </div>
            <div style="flex: 1;" data-product="${detailJson}">
              ...
              <div style="display:flex;flex-direction:column;gap:8px;">
                <button class="btn btn-outline small-btn" style="color:#94a3b8;border-color:#334155;width:100%;" onclick="egShowProductDetail(JSON.parse(this.closest('[data-product]').dataset.product), currentUser?.id)">👁 Preview</button>
                <div style="display:flex;gap:8px;">
                  <button class="btn btn-outline small-btn" style="color:#F59E0B;border-color:#F59E0B;flex:1;" onclick="editVendorProduct('${product._cloudId}')">Edit</button>
                  <button class="btn btn-danger small-btn" style="flex:1;" onclick="deleteVendorProduct('${product._cloudId}')">Delete</button>
                </div>
              </div>
            </div>
          </div>`;
...
function routeToPortal(role) {
  ...
  // Admin route: ?view=admin — only works if logged in as admin
  if (window._adminRouteRequested && currentUser && currentUser.is_admin) {
    window._adminRouteRequested = false;
    egAdminSetTab('pending');
    egAdminSetRole('all');
...
async function supabaseSubmitMarketplaceLead(lead) {
  try {
    ...
    function openPasswordResetScreen(role) {
      ...
      if (role === 'user') {
        ...
        // Corporate HQ partners use their own forgot-password screen
        // (corporate.js: corporateForgotPassword / corporateForgotStep3)
        showScreen('corporate-forgot-password');
        ...
        const newPwd = document.getElementById('corpForgotNewPassword');
        const confirmPwd = document.getElementById('corpForgotConfirmPassword');
        ...
        // Unknown role — default to the consumer reset screen rather than
        // silently dropping the user into an unrelated portal's flow.
        showScreen('user-forgot-password');
        const s1 = document.getElementById('userForgotStep1');
        const s3 = document.getElementById('userForgotStep3');
        if (s1) s1.style.display = 'none';
    ...
    async function registerVendor() {
      ...
      let phone = document.getElementById('regVendorPhone').value.trim();
      ...
      const cacDocFile = document.getElementById('regVendorCacDoc')?.files?.[0] || null;
      ...
      else if (!validateNigerianPhone(phone).valid) { document.getElementById('regVendorPhoneError').textContent = 'Enter a valid Nigerian number, e.g. 080XXXXXXXX'; hasError = true; }
      ...
      if (!document.getElementById('regVendorConsent')?.checked) {
        document.getElementById('regVendorConsentError').textContent = 'Please agree to the Privacy Policy to continue';
        hasError = true;
      }
      
      if (hasError) return;
      phone = validateNigerianPhone(phone).normalized || phone;
      ...
      if (result.success) {
        ...
        currentRole = 'vendor';
        // Upload CAC document if provided — fail silently so signup isn't blocked
        if (cacDocFile && result.user && supabaseClient?.storage) {
          try {
            const ext = cacDocFile.name.split('.').pop();
            const path = `cac-documents/${result.user.id}/cac_${Date.now()}.${ext}`;
            const { data: uploadData, error: uploadErr } = await supabaseClient.storage
              .from('documents').upload(path, cacDocFile, { upsert: true });
            if (!uploadErr && uploadData) {
              const { data: urlData } = supabaseClient.storage.from('documents').getPublicUrl(path);
              const cac_document_url = urlData?.publicUrl || null;
              if (cac_document_url) {
                await supabaseClient.from('profiles').update({ cac_document_url }).eq('id', result.user.id);
              }
            }
          } catch (uploadEx) {
            console.warn('CAC doc upload failed, continuing without it:', uploadEx);
          }
        }
        ...
        if (isExisting) {
          ...
          // Suppress auto-route-to-portal while we sign in to check/merge,
          // so a pre-existing Installer profile doesn't hijack navigation.
          window._egSuppressAutoRoute = true;
          ...
          try {
            ...
            if (existingUser) {
              ...
              showLoading(false);
              document.getElementById('regVendorEmailError').textContent = 'Email registered — enter its existing password above';
              showToast('This email already has an account. Enter its existing password above to add Vendor access, or <a href="javascript:void(0)" onclick="showScreen(\'vendor-forgot-password\')" style="color:#fff;text-decoration:underline;font-weight:700;">reset your password</a>.', 'error');
            }
          } catch(addErr) {
            showLoading(false);
            document.getElementById('regVendorEmailError').textContent = 'Email registered — enter its existing password above';
        ...
        showLoading(false);
        console.error('registerVendor error:', outerErr);
        showToast('DEBUG: ' + (outerErr?.message || String(outerErr)), 'error');
      }
    }
    ...
    async function registerInstaller() {
      ...
      let phone = document.getElementById('regInstPhone').value.trim();
      ...
      else if (!validateNigerianPhone(phone).valid) { document.getElementById('regInstPhoneError').textContent = 'Enter a valid Nigerian number, e.g. 080XXXXXXXX'; hasError = true; }
      ...
      if (!document.getElementById('regInstConsent')?.checked) {
        document.getElementById('regInstConsentError').textContent = 'Please agree to the Privacy Policy to continue';
        hasError = true;
      }
      ...
      if (result.success) {
        ...
        if (isExisting) {
          ...
          // Suppress auto-route-to-portal while we sign in to check/merge,
          // so a pre-existing Vendor profile doesn't hijack navigation.
          window._egSuppressAutoRoute = true;
          ...
          try {
            ...
            if (existingUser) {
              ...
              showLoading(false);
              document.getElementById('regInstEmailError').textContent = 'Email registered — enter its existing password above';
              showToast('This email already has an account. Enter its existing password above to add Installer access, or <a href="javascript:void(0)" onclick="showScreen(\'installer-forgot-password\')" style="color:#fff;text-decoration:underline;font-weight:700;">reset your password</a>.', 'error');
            }
          } catch(addErr) {
            showLoading(false);
            document.getElementById('regInstEmailError').textContent = 'Email registered — enter its existing password above';
        ...
        showLoading(false);
        console.error('registerInstaller error:', outerErr);
    ...
    // ── Google OAuth profile completion (Issue 2) ─────────────────
    // Called after Google sign-in for 'user' role when phone or state
    // is missing. Shows a lightweight overlay — no full registration form.
    function egShowGoogleProfileCompletion(userId, profile) {
      const stateOptions = [
        'Abia','Adamawa','Akwa Ibom','Anambra','Bauchi','Bayelsa','Benue','Borno',
        'Cross River','Delta','Ebonyi','Edo','Ekiti','Enugu','FCT','Gombe','Imo',
        ...
        'Yobe','Zamfara'
      ].map(s => '<option value="' + s + '"' + (profile && profile.state === s ? ' selected' : '') + '>' + s + '</option>').join('');

      const overlay = document.createElement('div');
      overlay.id = 'egGoogleCompleteOverlay';
      overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.8);display:flex;align-items:center;justify-content:center;z-index:9999;padding:24px;';
      overlay.innerHTML = `
        <div style="background:#0f172a;border:1px solid #1e3a5f;border-radius:20px;padding:32px 24px;max-width:360px;width:100%;">
          <div style="text-align:center;margin-bottom:20px;">
            <div style="font-size:36px;margin-bottom:8px;">👋</div>
            <h2 style="font-size:18px;font-weight:700;color:#f3f4f6;margin:0 0 6px;">Almost there!</h2>
            <p style="font-size:13px;color:#94a3b8;margin:0;">Just a couple more details so we can match you with installers and vendors near you.</p>
          </div>
          <div style="margin-bottom:14px;">
            <label style="display:block;font-size:12px;font-weight:600;color:#94a3b8;margin-bottom:6px;text-transform:uppercase;letter-spacing:0.05em;">Phone / WhatsApp *</label>
            <input type="tel" id="egGCPhone" placeholder="08012345678" value="${escapeHtml(profile && (profile.phone || profile.whatsapp_number) || '')}" style="width:100%;padding:12px;border-radius:10px;border:1px solid #1e3a5f;background:#0a0e17;color:#f3f4f6;font-size:14px;box-sizing:border-box;"/>
            <div id="egGCPhoneErr" style="color:#ef4444;font-size:11px;margin-top:4px;"></div>
          </div>
          <div style="margin-bottom:24px;">
            <label style="display:block;font-size:12px;font-weight:600;color:#94a3b8;margin-bottom:6px;text-transform:uppercase;letter-spacing:0.05em;">State *</label>
            <select id="egGCState" style="width:100%;padding:12px;border-radius:10px;border:1px solid #1e3a5f;background:#0a0e17;color:#f3f4f6;font-size:14px;box-sizing:border-box;">
              <option value="">Select State</option>
              ${stateOptions}
            </select>
            <div id="egGCStateErr" style="color:#ef4444;font-size:11px;margin-top:4px;"></div>
          </div>
          <button onclick="egSubmitGoogleProfileCompletion('${escapeHtml(userId)}')" style="width:100%;padding:14px;background:linear-gradient(135deg,#f59e0b,#d97706);border:none;border-radius:12px;color:#111;font-size:15px;font-weight:800;cursor:pointer;">
            Continue →
          </button>
        </div>`;
      document.body.appendChild(overlay);
    ...
    window.egSubmitGoogleProfileCompletion = async function (userId) {
      const phone = (document.getElementById('egGCPhone')?.value || '').trim();
      const state = (document.getElementById('egGCState')?.value || '');
      let valid = true;
      if (!phone) { const e = document.getElementById('egGCPhoneErr'); if (e) e.textContent = 'Phone required'; valid = false; }
      if (!state) { const e = document.getElementById('egGCStateErr'); if (e) e.textContent = 'State required'; valid = false; }
      if (!valid) return;

      showLoading(true, 'Saving...');
      try {
        await supabaseClient.from('profiles').upsert([{
          id: userId, phone: phone, whatsapp_number: phone, state: state
        }], { onConflict: 'id' });
        if (currentUser) { currentUser.phone = phone; currentUser.state = state; }
        showLoading(false);
        const overlay = document.getElementById('egGoogleCompleteOverlay');
        if (overlay) overlay.remove();
        egShowUserDashboard();
        showToast('Profile saved!', 'success');
      } catch (err) {
        showLoading(false);
        showToast('Could not save — ' + (err.message || 'try again'), 'error');
      }
    };
    ...
    async function handleGoogleAuthReturn() {
      try {
        ...
        // Google sign-in for 'user' role: if phone or state is missing,
        // show a lightweight completion prompt before going to dashboard.
        if (resolvedRole === 'user') {
          const hasPhone = !!(activeProfile && (activeProfile.phone || activeProfile.whatsapp_number));
          const hasState = !!(activeProfile && activeProfile.state);
          if (!hasPhone || !hasState) {
            egShowGoogleProfileCompletion(session.user.id, activeProfile);
            return;
          }
        }
    ...
    // Initialize app — restore session on startup
    // Admin route: ?view=admin — only works if logged in as admin
    (async () => {
      try {
        const urlParams = new URLSearchParams(window.location.search);
        const isAdminRoute = urlParams.get('view') === 'admin';
        if (!supabaseClient) return;
        const { data: { session } } = await supabaseClient.auth.getSession();
        if (session && session.user) {
          await restoreEnergyGuideUserSession(session);
          // Admin deep-link: route to admin panel if user is admin and ?view=admin
          if (isAdminRoute) {
            const { data: profile } = await supabaseClient.from('profiles').select('is_admin').eq('id', session.user.id).maybeSingle();
        ...
        console.warn('Session restore on startup failed:', e);
      }
    })();
    
    window.addEventListener('unhandledrejection', function(event) {
      console.error('Unhandled promise rejection:', event.reason);
      // Don't show generic toast if a more specific error handler is active
      // or if the loading overlay already dismissed with a timeout warning
      const loadingActive = document.getElementById('loadingOverlay')?.classList.contains('active');
      if (!loadingActive && typeof showToast === 'function') {
  ...
  // admin route handled via URL parameter only — no menu item
      egCloseMenu();
      if (action === 'apk') {
        egShowMenuModal(
          '📲',
    ...
    function egShowMenuModal(icon, title, body, cta) {
      // Reuse the shared egShowModal helper if available, else build our own
      const inner = `
