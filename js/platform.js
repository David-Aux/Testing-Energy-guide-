// EG-VERSION: v304-fix
    ...
    async function routeToPortalWithCheck(role) {
      ...
      if (role === 'installer') {
        ...
        if (!isInstallerProfileComplete(currentUser)) {
          // Missing core fields — force profile completion first, regardless of sign-in method
          prefillCompleteInstallerProfile(currentUser);
          showScreen('complete-installer-profile');
        } else if (instStatus === 'pending' || !instStatus) {
          // Profile complete but not yet approved — hold at pending screen
        ...
        if (!isVendorProfileComplete(currentUser)) {
          // Missing core fields — force profile completion first
          prefillCompleteVendorProfile(currentUser);
          showScreen('complete-vendor-profile');
        } else if (vendStatus === 'pending' || !vendStatus) {
          showScreen('pending-review');
    ...
    // ── Expose functions used in dynamically-generated onclick="" HTML ──────
    // These live inside the platform IIFE/closure, so the browser can't reach
    // them from inline event handlers unless we pin them to window explicitly.
    window.showToast                  = showToast;
    window.showLoading                = showLoading;
    window.renderJobLeads             = renderJobLeads;
    window.renderVendorRequests       = renderVendorRequests;
    window.openQuoteBuilderFromLead   = openQuoteBuilderFromLead;
    window.openOfferBuilderFromLead   = openOfferBuilderFromLead;
    window.contactLeadPhone           = contactLeadPhone;
    window.contactLeadEmail           = contactLeadEmail;
    window.contactCustomer            = contactCustomer;
    window.updateLeadStatus           = updateLeadStatus;
    window.closeInstallerLead         = closeInstallerLead;
    window.markVendorResponded        = markVendorResponded;
    window.claimLead                  = claimLead;
    window.loadCalculation            = loadCalculation;
    window.deleteCalc                 = deleteCalc;
    window.prepareLeadFlow            = prepareLeadFlow;
    window.selectInstallerForQuote    = selectInstallerForQuote;
    window.selectVendorForOffer       = selectVendorForOffer;
    window.toggleMarketplaceProfileDetails = toggleMarketplaceProfileDetails;
    window.egAdminApproveProfile      = egAdminApproveProfile;
    window.egAdminRejectProfile       = egAdminRejectProfile;
    window.editVendorProduct          = editVendorProduct;
    window.deleteVendorProduct        = deleteVendorProduct;
    window.loadUserSavedCalculation   = loadUserSavedCalculation;
    window.downloadSavedSystemPDF     = downloadSavedSystemPDF;
    window.shareSavedSystemWhatsApp   = shareSavedSystemWhatsApp;
    window.deleteUserSavedCalculation = deleteUserSavedCalculation;
...
// ================================================================
    ...
    async function handleInstallerLogin() {
      ...
      if (result.success && isInstaller) {
        egSyncUser({ ...result.user, ...instProfile });
        currentRole = 'installer';
      ...
      if (result.success && isVendor) {
        egSyncUser({ ...result.user, ...vendProfile });
        currentRole = 'vendor';
    ...
    async function renderVendorRequests() {
      ...
      container.innerHTML = data.map(lead => {
        ...
        <div class="lead-card">
          ...
          <div style="font-size:13px;color:#687076;margin-bottom:6px;">
            ${isUnlocked ? `${phone || 'No phone'}${email ? ' &nbsp;·&nbsp; ' + email : ''}` : '📵 Contact visible after unlock'}
            ${location ? ' &nbsp;·&nbsp; ' + location : ''}
          </div>
          ${snapshot ? `<div style="font-size:12px;color:#374151;margin-bottom:6px;">${snapshot}</div>` : ''}
          ${extras   ? `<div style="font-size:12px;color:#687076;margin-bottom:6px;">${extras}</div>` : ''}
