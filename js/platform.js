// EG-VERSION: v304-fix
    ...
    async function routeToPortalWithCheck(role) {
      currentRole = role;
      if (!currentUser) { uaOpenForRole(role); return; }
    ...
    async function saveCompleteInstallerProfile() {
      ...
      try {
        ...
        const payload = {
          ...
          status: 'approved',
          installer_status: 'approved'
        };
        const { error } = await supabaseClient.from('profiles').upsert([payload], { onConflict: 'id' });
        showLoading(false);
        if (error) throw error;
        currentUser = { ...currentUser, company_name: business || display_name, phone, state, city, status: 'approved', installer_status: 'approved' };
    ...
    async function saveCompleteVendorProfile() {
      ...
      try {
        ...
        showLoading(false);
        if (error) throw error;
        currentUser = { ...currentUser, company_name: business || display_name, phone, state, city, status: 'approved', vendor_status: 'approved' };
    ...
    function egBuildCostBreakdown(result) {
      ...
      const pvBreakerPrice = EG_PV_BREAKER_PRICES[Number(result.pvBreaker)] || 0;
      const acBreakerPrice = EG_AC_BREAKER_PRICES[Number(result.acBreaker)] || 0;
      const pvCablePerMeter = EG_PV_CABLE_PRICES[String(result.pvCable)] || EG_PV_CABLE_PRICES[Number(result.pvCable)] || 0;
      const acCablePerMeter = EG_AC_CABLE_PRICES[String(result.acCable)] || EG_AC_CABLE_PRICES[Number(result.acCable)] || 0;
      const pvCableCost = pvCablePerMeter * EG_CABLE_LENGTHS.pv;
      const acCableCost = acCablePerMeter * EG_CABLE_LENGTHS.ac;
      // Parallel battery string logic
      const sysVolt = result.sysV || (result.invKva <= 1.5 ? 12 : result.invKva <= 3.5 ? 24 : 48);
      const maxBattA = (result.invKva * 1000) / sysVolt;
      const bp = egBattParallel(maxBattA);
      const numBattStrings      = bp.numStrings;
      const battStringBreaker   = bp.breakerPerString;
      const battStringCable     = bp.cableSize;
      const batteryBreakerPrice = (EG_BATTERY_BREAKER_PRICES[battStringBreaker] || 0) * numBattStrings;
      const batteryCablePerMeter = EG_BATTERY_CABLE_PRICES[String(battStringCable)] || EG_BATTERY_CABLE_PRICES[Number(battStringCable)] || 0;
      const batteryCableCost    = batteryCablePerMeter * EG_CABLE_LENGTHS.battery * numBattStrings;
      ...
      return {
        ...
        numBattStrings, battStringBreaker, battStringCable,
        totalCost
      };
    }
    ...
    function openInstallerMarketplace() {
      // Gate: if anonymous user portal visitor, prompt sign-up before showing lead-generating action
      if (currentRole === 'user' && !isSignedInEndUser()) {
    ...
    function openVendorMarketplace() {
      // Vendors are not lead-gated — browsing products is free friction
      selectedVendorId = null;
      selectedVendorProfile = null;
    ...
    function selectRole(role) {
      ...
      topBar.className = 'top-bar ' + (role === 'ci' ? 'user' : role);

      if (role === 'user') {
        if (typeof updateUserAccountUI === 'function') updateUserAccountUI();
        showScreen('user-calculator');
      } else if (role === 'ci') {
        // Gate C&I behind a lightweight sign-up
        if (isSignedInEndUser() || currentUser) {
          showScreen('ci-calculator');
        } else {
          ciAuthSetMode('signin');
          showScreen('ci-auth');
        }
      } else if (role === 'installer' || role === 'vendor') {
        uaOpenForRole(role);
      }
    }
    ...
    function showScreen(screenId) {
      ...
      // Show sign-up nudge on results screen for anonymous users
      if (screenId === 'user-results') {
        const nudge = document.getElementById('userResultsSignupNudge');
        if (nudge) nudge.style.display = isSignedInEndUser() ? 'none' : 'block';
      }
    }
    ...
    function updateUserAccountUI() {
      ...
      // Hide results nudge if now signed in
      const nudge = document.getElementById('userResultsSignupNudge');
      if (nudge) nudge.style.display = isSignedInEndUser() ? 'none' : 'block';
    ...
    function welcomeOpenSignup() {
      // No role pre-selected — user picks inside the signup form
      window._uaRole = null;
      _uaRenderWelcomeBanner();
      uaSetMode('signup');
      showScreen('unified-auth');
    }

    function welcomeOpenSignin() {
      // Sign in — no role needed upfront, we detect from profile after login
      window._uaRole = null;
      _uaRenderWelcomeBanner();
      uaSetMode('signin');
      showScreen('unified-auth');
    ...
    function _uaRenderWelcomeBanner() {
      // Generic banner when role not yet known
      const banner = document.getElementById('unifiedAuthRoleBanner');
      if (banner) { banner.style.background = 'rgba(10,126,164,0.08)'; banner.style.border = '1px solid rgba(10,126,164,0.2)'; }
      const emojiEl = document.getElementById('unifiedAuthRoleEmoji');
      if (emojiEl) emojiEl.textContent = '☀️';
      const titleEl = document.getElementById('unifiedAuthRoleTitle');
      if (titleEl) { titleEl.textContent = 'Energy Guide'; titleEl.style.color = '#0a7ea4'; }
      const subtitleEl = document.getElementById('unifiedAuthRoleSubtitle');
      if (subtitleEl) { subtitleEl.textContent = 'Nigeria\'s Solar Platform'; subtitleEl.style.color = '#0a7ea4'; }
    }

    // Role picker inside signup form
    const UA_ROLE_COLORS = { user: '#0a7ea4', installer: '#22C55E', vendor: '#F59E0B', ci: '#0ea5e9' };

    function uaPickRole(role) {
      window._uaRole = role === 'ci' ? 'user' : role; // C&I users are 'user' type in DB
      window._uaCiFlow = role === 'ci';

      // Highlight selected card
      ['user','installer','vendor','ci'].forEach(r => {
        const el = document.getElementById('uaRole' + r.charAt(0).toUpperCase() + r.slice(1));
        if (!el) return;
        const color = UA_ROLE_COLORS[r];
        if (r === role) {
      ...
      // Show correct field section
      const userFields = document.getElementById('uaUserFields');
      const instFields = document.getElementById('uaInstallerFields');
      const vendFields = document.getElementById('uaVendorFields');
      const googleRow  = document.getElementById('uaGoogleSignupRow');
      if (userFields) userFields.style.display = (role === 'user' || role === 'ci') ? 'block' : 'none';
      if (instFields) instFields.style.display = role === 'installer' ? 'block' : 'none';
      if (vendFields) vendFields.style.display = role === 'vendor' ? 'block' : 'none';
      if (googleRow)  googleRow.style.display  = (role === 'user' || role === 'ci') ? 'block' : 'none';

      // Populate states if needed
      if (role === 'installer') _uaPopulateStates('uaInstState');
      if (role === 'vendor')    _uaPopulateStates('uaVendState');

      // Update sign up button colour
      const signupBtn = document.getElementById('uaSignupBtn');
      if (signupBtn) {
        signupBtn.style.background = UA_ROLE_COLORS[role] || '#0a7ea4';
        signupBtn.style.borderColor = UA_ROLE_COLORS[role] || '#0a7ea4';
      }
    }
    ...
    const UA_ROLE_CONFIG = {
      installer: {
        emoji: '🛠',
        title: 'Installer Portal',
        subtitle: 'Professional calculator, leads & quote builder',
        color: '#22C55E',
      ...
      vendor: {
        emoji: '🏪',
        title: 'Vendor Portal',
        subtitle: 'Product management & customer requests',
        color: '#F59E0B',
      ...
      user: {
        emoji: '🔋',
        title: 'User Account',
        subtitle: 'Save systems, track requests',
        color: '#0a7ea4',
    ...
    function uaOpenForRole(role) {
      window._uaRole = role;
      const cfg = UA_ROLE_CONFIG[role] || UA_ROLE_CONFIG.user;

      // Style banner
      const banner = document.getElementById('unifiedAuthRoleBanner');
      if (banner) { banner.style.background = cfg.bg; banner.style.border = '1px solid ' + cfg.color + '33'; }
      const emojiEl = document.getElementById('unifiedAuthRoleEmoji');
      if (emojiEl) emojiEl.textContent = cfg.emoji;
      const titleEl = document.getElementById('unifiedAuthRoleTitle');
      if (titleEl) { titleEl.textContent = cfg.title; titleEl.style.color = cfg.color; }
      const subtitleEl = document.getElementById('unifiedAuthRoleSubtitle');
      if (subtitleEl) { subtitleEl.textContent = cfg.subtitle; subtitleEl.style.color = cfg.color; }

      // Style buttons
      const signinBtn = document.getElementById('uaSigninBtn');
      const signupBtn = document.getElementById('uaSignupBtn');
      if (signinBtn) signinBtn.className = 'btn ' + cfg.btnClass;
      if (signupBtn) signupBtn.className = 'btn ' + cfg.btnClass;

      // Populate state dropdowns for installer/vendor
      if (role === 'installer') {
        _uaPopulateStates('uaInstState');
      } else if (role === 'vendor') {
        _uaPopulateStates('uaVendState');
      }

      // Default to signin if user has chosen this portal before, else signup
      uaSetMode('signin');
      showScreen('unified-auth');
    }

    function _uaPopulateStates(selectId) {
    ...
    function uaSetMode(mode) {
      ...
      const role = window._uaRole || null;
      const cfg = role ? (UA_ROLE_CONFIG[role] || UA_ROLE_CONFIG.user) : { color: '#0a7ea4', btnClass: 'btn-primary' };

      // Tabs
      const tabSI = document.getElementById('uaTabSignin');
      const tabSU = document.getElementById('uaTabSignup');
      if (tabSI) { tabSI.style.background = isSignup ? 'transparent' : cfg.color; tabSI.style.color = isSignup ? '' : '#fff'; tabSI.style.borderColor = isSignup ? '#334155' : cfg.color; }
      if (tabSU) { tabSU.style.background = isSignup ? cfg.color : 'transparent'; tabSU.style.color = isSignup ? '#fff' : ''; tabSU.style.borderColor = isSignup ? cfg.color : '#334155'; }

      // Forms
      const signinForm = document.getElementById('uaSigninForm');
      const signupForm = document.getElementById('uaSignupForm');
      if (signinForm) signinForm.style.display = isSignup ? 'none' : 'block';
      if (signupForm) signupForm.style.display = isSignup ? 'block' : 'none';

      // Role picker — always visible on signup; hidden on signin
      const rolePicker = document.getElementById('uaRolePicker');
      if (rolePicker) rolePicker.style.display = isSignup ? 'block' : 'none';

      // Field sections — only show if role already picked
      const userFields = document.getElementById('uaUserFields');
      const instFields = document.getElementById('uaInstallerFields');
      const vendFields = document.getElementById('uaVendorFields');
      const googleRow  = document.getElementById('uaGoogleSignupRow');
      if (userFields) userFields.style.display = (isSignup && role === 'user') ? 'block' : 'none';
      if (instFields) instFields.style.display = (isSignup && role === 'installer') ? 'block' : 'none';
      if (vendFields) vendFields.style.display = (isSignup && role === 'vendor') ? 'block' : 'none';
      if (googleRow)  googleRow.style.display  = (isSignup && (role === 'user' || !role)) ? 'block' : 'none';

      const signinErr = document.getElementById('uaSigninError');
      const signupErr = document.getElementById('uaSignupError');
      if (signinErr) signinErr.style.display = 'none';
      if (signupErr) signupErr.style.display = 'none';
    }

    function uaGoForgotPassword() {
      const role = window._uaRole || 'installer';
      // Route to existing forgot password screen for the role
      if (role === 'installer') showScreen('installer-forgot-password');
      else if (role === 'vendor') showScreen('vendor-forgot-password');
      else openUserAuthModal('signup'); // user portal uses modal
    }

    async function uaHandleSignin() {
      const role = window._uaRole || null;
      const email = (document.getElementById('uaSigninEmail').value || '').trim().toLowerCase();
      const password = document.getElementById('uaSigninPassword').value || '';
      const errEl = document.getElementById('uaSigninError');
      ...
      // Detect role from profile if not pre-selected (welcome → sign in flow)
      const detectedRole = role || (profile.is_installer ? 'installer' : profile.is_vendor ? 'vendor' : 'user');

      currentUser = { ...result.user, ...profile };
      currentRole = detectedRole;
      onLoginSuccess().catch(() => {});

      if (detectedRole === 'installer') {
      ...
      document.getElementById('uaSigninEmail').value = '';
      document.getElementById('uaSigninPassword').value = '';
    }

    async function uaHandleSignup() {
      const role = window._uaRole || null;
      const errEl = document.getElementById('uaSignupError');
      errEl.style.display = 'none';

      if (!role) {
        errEl.textContent = 'Please select your account type above before continuing.';
        errEl.style.display = 'block';
        document.getElementById('uaRolePicker').scrollIntoView({ behavior: 'smooth', block: 'start' });
        return;
      }

      // Clear previous inline errors
      document.querySelectorAll('#unified-auth .error-message').forEach(e => e.textContent = '');

      let email, password, passwordConfirm, userData;
      let hasError = false;

      const setErr = (id, msg) => { const el = document.getElementById(id); if (el) el.textContent = msg; hasError = true; };
      const val = id => { const el = document.getElementById(id); return el ? el.value.trim() : ''; };

      if (role === 'installer') {
        const name = val('uaInstName'), company = val('uaInstCompany');
        email = val('uaInstEmail').toLowerCase();
        const phone = val('uaInstPhone'), state = val('uaInstState'), city = val('uaInstCity');
        password = document.getElementById('uaInstPassword').value;
        passwordConfirm = document.getElementById('uaInstPasswordConfirm').value;

        if (!name) setErr('uaInstNameErr', 'Full name required');
        if (!company) setErr('uaInstCompanyErr', 'Business name required');
        if (!email) setErr('uaInstEmailErr', 'Email required');
        else if (!isValidEmail(email)) setErr('uaInstEmailErr', 'Invalid email');
        if (!phone) setErr('uaInstPhoneErr', 'Phone required');
        if (!state) setErr('uaInstStateErr', 'State required');
        if (!city) setErr('uaInstCityErr', 'City required');
        if (!password) setErr('uaInstPasswordErr', 'Password required');
        else if (!isValidPassword(password)) setErr('uaInstPasswordErr', 'Min 8 characters');
        if (password !== passwordConfirm) setErr('uaInstPasswordConfirmErr', 'Passwords do not match');
        if (hasError) return;

        userData = { role: 'installer', full_name: name, company_name: company, business_name: company, phone, state, city, service_area: city,
          whatsapp_number: val('uaInstWhatsapp'), years_experience: val('uaInstYears') };

      } else if (role === 'vendor') {
        const name = val('uaVendName'), company = val('uaVendCompany');
        email = val('uaVendEmail').toLowerCase();
        const phone = val('uaVendPhone'), state = val('uaVendState'), city = val('uaVendCity');
        password = document.getElementById('uaVendPassword').value;
        passwordConfirm = document.getElementById('uaVendPasswordConfirm').value;

        if (!name) setErr('uaVendNameErr', 'Full name required');
        if (!company) setErr('uaVendCompanyErr', 'Business name required');
        if (!email) setErr('uaVendEmailErr', 'Email required');
        else if (!isValidEmail(email)) setErr('uaVendEmailErr', 'Invalid email');
        if (!phone) setErr('uaVendPhoneErr', 'Phone required');
        if (!state) setErr('uaVendStateErr', 'State required');
        if (!city) setErr('uaVendCityErr', 'City required');
        if (!password) setErr('uaVendPasswordErr', 'Password required');
        else if (!isValidPassword(password)) setErr('uaVendPasswordErr', 'Min 8 characters');
        if (password !== passwordConfirm) setErr('uaVendPasswordConfirmErr', 'Passwords do not match');
        if (hasError) return;

        userData = { role: 'vendor', full_name: name, company_name: company, business_name: company, phone, state, city,
          delivery_coverage: val('uaVendDelivery'),
          sells_panels: document.getElementById('uaVendPanels').checked,
          sells_inverters: document.getElementById('uaVendInverters').checked,
          sells_lithium: document.getElementById('uaVendLithium').checked,
          sells_accessories: document.getElementById('uaVendAccessories').checked };

      } else {
        // user portal
        const name = val('uaUserName');
        email = val('uaUserEmail').toLowerCase();
        password = document.getElementById('uaUserPassword').value;
        if (!name || !email || !password) { errEl.textContent = 'All fields are required.'; errEl.style.display = 'block'; return; }
        if (!isValidEmail(email)) { errEl.textContent = 'Invalid email.'; errEl.style.display = 'block'; return; }
        if (!isValidPassword(password)) { errEl.textContent = 'Password must be at least 8 characters.'; errEl.style.display = 'block'; return; }
        userData = { role: 'user', full_name: name };
      }
      ...
      showToast('Account created! Welcome to Energy Guide.', 'success');
      ...
      if (loginResult.success) {
        ...
        currentRole = role === 'ci' ? 'user' : role;
        onLoginSuccess().catch(() => {});
        if (role === 'installer') await routeToPortalWithCheck('installer');
        else if (role === 'vendor') await routeToPortalWithCheck('vendor');
        else { if (typeof updateUserAccountUI === 'function') updateUserAccountUI(); showScreen('user-calculator'); }
      } else {
        uaSetMode('signin');
      }
    }
    ...
    function ciAuthSetMode(mode) {
      const isSignup = mode === 'signup';
      const tabSI = document.getElementById('ciAuthTabSignin');
      const tabSU = document.getElementById('ciAuthTabSignup');
      if (tabSI) { tabSI.style.background = isSignup ? 'transparent' : '#0ea5e9'; tabSI.style.color = isSignup ? '' : '#fff'; tabSI.style.borderColor = isSignup ? '#334155' : '#0ea5e9'; }
      if (tabSU) { tabSU.style.background = isSignup ? '#0ea5e9' : 'transparent'; tabSU.style.color = isSignup ? '#fff' : ''; tabSU.style.borderColor = isSignup ? '#0ea5e9' : '#334155'; }

      const nameGrp = document.getElementById('ciAuthNameGroup');
      const coGrp = document.getElementById('ciAuthCompanyGroup');
      const btn = document.getElementById('ciAuthSubmitBtn');
      if (nameGrp) nameGrp.style.display = isSignup ? 'block' : 'none';
      if (coGrp) coGrp.style.display = isSignup ? 'block' : 'none';
      if (btn) btn.textContent = isSignup ? 'Create Account & Continue' : 'Sign In';
      window._ciAuthMode = mode;

      const errEl = document.getElementById('ciAuthError');
      if (errEl) errEl.style.display = 'none';
    }

    async function ciAuthHandleSubmit() {
      const mode = window._ciAuthMode || 'signin';
      const email = (document.getElementById('ciAuthEmail').value || '').trim().toLowerCase();
      const password = document.getElementById('ciAuthPassword').value || '';
      const errEl = document.getElementById('ciAuthError');
      errEl.style.display = 'none';

      if (!email || !password) { errEl.textContent = 'Please enter your email and password.'; errEl.style.display = 'block'; return; }

      showLoading(true, mode === 'signup' ? 'Creating account...' : 'Signing in...');

      if (mode === 'signup') {
        const name = (document.getElementById('ciAuthName').value || '').trim();
        const company = (document.getElementById('ciAuthCompany').value || '').trim();
        const result = await supabaseSignUp(email, password, { role: 'user', full_name: name || email, company_name: company || null });
        showLoading(false);
        if (!result.success) {
          if (result.error === 'already_registered') { errEl.textContent = 'Email already registered. Please sign in.'; ciAuthSetMode('signin'); }
          else errEl.textContent = result.error || 'Registration failed.';
          errEl.style.display = 'block';
          return;
        }
        if (result.emailConfirmationRequired) {
          showToast('Account created! Check your email to confirm, then sign in here.', 'success');
          ciAuthSetMode('signin');
          return;
        }
      }
      ...
      if (!loginResult.success) {
        errEl.textContent = loginResult.error || 'Invalid email or password.';
        errEl.style.display = 'block';
        return;
      }

      currentUser = { ...loginResult.user, ...(loginResult.profile || {}) };
      currentRole = 'user';
      onLoginSuccess().catch(() => {});
      if (typeof updateUserAccountUI === 'function') updateUserAccountUI();
      showScreen('ci-calculator');
      showToast('Welcome! Your C&I sizing tool is ready.', 'success');
    }
    ...
    function downloadSystemPDF() {
      ...
      setTimeout(() => {
        try {
          ...
          const _rnbs = r.numBattStrings || 1;
          const _rbsb = r.battStringBreaker || r.battBreaker || '-';
          const _rbsc = r.battStringCable   || r.battCable   || '-';
          const _rBrkDetail = _rnbs > 1 ? `${_rnbs}× ${_rbsb}A DC (parallel)` : `${_rbsb}A DC`;
          const _rCabDetail = _rnbs > 1 ? `${_rnbs}× ${_rbsc}mm² (parallel)` : `${_rbsc}mm²`;
          const refRows = [
            ['PV Breaker',      `${r.pvBreaker || '-'}A DC`,   M(r.pvBreakerPrice)],
            ['Battery Breaker', _rBrkDetail,                    M(r.batteryBreakerPrice)],
            ['AC Breaker',      `${r.acBreaker || '-'}A AC`,   M(r.acBreakerPrice)],
            ['PV Cable',        `${r.pvCable || '-'}mm²`,      M(r.pvCablePerMeter) + '/m'],
            ['Battery Cable',   _rCabDetail,                    M(r.batteryCablePerMeter) + '/m'],
  ...
  try {
    ...
    const { error: profileError } = _safeRole ? await supabaseClient
      ...
      .upsert([{
        ...
        status: (userData.role === 'installer' || userData.role === 'vendor') ? 'approved' : null,
        installer_status: userData.role === 'installer' ? 'approved' : null,
        vendor_status: userData.role === 'vendor' ? 'approved' : null,
...
async function supabaseSubmitMarketplaceLead(lead) {
  try {
    ...
    async function registerVendor() {
      ...
      if (result.success) {
        currentUser = { ...(result.user || {}), role: 'vendor', company_name: company, full_name: fullName, phone, state, city, status: 'approved', vendor_status: 'approved' };
        currentRole = 'vendor';
          ...
          try {
            ...
            if (existingUser) {
              ...
              if (existingProfile && existingProfile.is_vendor) {
                ...
                showLoading(false);
                currentUser = { ...existingUser, ...(existingProfile || {}), is_vendor: true, vendor_status: 'approved' };
                currentRole = 'vendor';
                egSendApplicationNotification('vendor', company || fullName, phone, state);
                showToast('Vendor access added to your account!', 'success');
                showScreen('vendor-dashboard');
              }
            } else {
              showLoading(false);
    ...
    async function registerInstaller() {
      ...
      if (result.success) {
        currentUser = { ...(result.user || {}), role: 'installer', company_name: company, full_name: fullName, phone, state, city: area, status: 'approved', installer_status: 'approved' };
        currentRole = 'installer';
          ...
          try {
            ...
            if (existingUser) {
              ...
              if (existingProfile && existingProfile.is_installer) {
                ...
                showLoading(false);
                currentUser = { ...existingUser, ...(existingProfile || {}), is_installer: true, installer_status: 'approved' };
                currentRole = 'installer';
                egSendApplicationNotification('installer', company || fullName, phone, state);
                showToast('Installer access added to your account!', 'success');
                showScreen('installer-dashboard');
              }
            } else {
              showLoading(false);
      ...
      try {
        ...
        if (hasOtpExpired) {
          ...
          if (isEmailConfirmation || (!isRecovery && !localStorage.getItem('pendingPasswordResetRole'))) {
            ...
            // Route to unified auth — role will be detected from profile after sign in
            welcomeOpenSignin();
          } else {
