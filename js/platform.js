// EG-VERSION: v304-fix
    ...
    // ═══════════════════════════════════════════════════════════
    // UNIFIED AUTH SCREEN — uaOpenForRole, uaSetMode, uaHandle*
    // ═══════════════════════════════════════════════════════════
    ...
    function uaSetMode(mode) {
      ...
      const cfg = UA_ROLE_CONFIG[role] || UA_ROLE_CONFIG.installer;
      ...
      // Role-specific signup field sections
      ...
      // Google signup only shown for user portal (installer/vendor use email so we capture data)
      if (googleRow) googleRow.style.display = (isSignup && role === 'user') ? 'block' : 'none';
    ...
    async function uaHandleSignin() {
      ...
      if (role === 'installer') {
        const isInstaller = profile.role === 'installer' || profile.is_installer === true;
        if (!isInstaller) { errEl.textContent = 'No installer account found for this email. Please create one.'; errEl.style.display = 'block'; return; }
        ...
        if (!isVendor) { errEl.textContent = 'No vendor account found for this email. Please create one.'; errEl.style.display = 'block'; return; }
        currentUser = { ...result.user, ...profile };
        currentRole = 'vendor';
        onLoginSuccess().catch(() => {});
