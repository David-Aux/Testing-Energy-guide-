(function(){
  ...
  // Flag set by calc engines so safeTop skips calc screens while results are showing
  window._egCalcScrollPending = false;

  function safeTop(){
    // Don't reset scroll on calc screens when a result scroll is in progress
    if (window._egCalcScrollPending) return;
    try {
