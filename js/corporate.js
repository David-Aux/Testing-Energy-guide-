...
(function () {
  ...
  let agentPortalCode = null;   // company_code for current agent session
  let agentInfo       = null;   // { code, name, phone, state, city }
  let agentCalcResult = null;   // result from real calc engine
  let agentLastApps   = [];     // appliances array from real calc engine

  // ── Supabase helper ────────────────────────────────────────────
  function getSupabase() {
    return (typeof supabaseClient !== 'undefined' && supabaseClient) ? supabaseClient : window.supabaseClient || null;
  }

  // ── URL param: ?agent=CODE → launch agent portal directly ─────
  ...
  function resetAgentPortal() {
    ...
    const postCalc = document.getElementById('agentPostCalcActions');
    if (postCalc) postCalc.style.display = 'none';
  ...
  window.corporateRegister = async function () {
    ...
    try {
      ...
      if (authErr) throw authErr;
      const { error: insErr } = await sb.from('corporate_partners').insert([{ user_id: authData.user.id, company_name: company, company_code: rawCode, contact_name: contact, email, industry }]);
  ...
  async function loadCorpStats() {
    ...
    } catch (err) { console.warn('Corporate stats error:', err.message); }
  }
  ...
  window.corporateDownloadReport = async function () {
    ...
    try {
      ...
      if (error) throw error;
      if (!data || data.length === 0) { if (typeof showLoading === 'function') showLoading(false); if (typeof showToast === 'function') showToast('No submissions to export yet', 'error'); return; }
      ...
      const rows = data.map(r => {
        ...
        return [r.agent_code||'', r.agent_name||'', r.phone||'', r.state||'', r.city||'',
          s.invKva ? s.invKva : '', s.numPanels||'', s.dailyKwh ? Number(s.dailyKwh).toFixed(2) : '',
      ...
      a.href     = url;
      a.download = corpSession.company_code + '_agents_' + new Date().toISOString().slice(0,10) + '.csv';
      a.click();
  ...
  window.agentGoToCalculator = function () {
    ...
    ].forEach(([id, fail, msg]) => { const el = document.getElementById(id); if (el) el.textContent = fail ? msg : ''; if (fail) valid = false; });
    if (!valid) return;
  ...
  // When agent is active: capture result + show agent submit instead of user actions
  ...
  window.showScreen = function (screenId) {
    ...
    if (screenId === 'corporate-agent-portal') {
      const userActions = document.getElementById('userPostCalcActions');
      if (userActions) userActions.style.display = 'none';
    }
  };
