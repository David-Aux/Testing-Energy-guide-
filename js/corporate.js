...
(function () {
  ...
  async function loadCorpSubmissions() {
    ...
    try {
      ...
      listEl.innerHTML = renderSubmissionCards(data);
  ...
  window.corporateDownloadReport = async function () {
    ...
    try {
      ...
      downloadCSV(data, corpSession.company_code + '_all_agents');
      if (typeof showLoading === 'function') showLoading(false);
    } catch (err) {
      if (typeof showLoading === 'function') showLoading(false);
  ...
  window.showScreen = function (screenId) {
    ...
    if (screenId === 'super-agent-dashboard' && saSession) loadSuperAgentStats();
    if (screenId === 'super-agent-submissions')            loadSuperAgentSubmissions();
    if (screenId === 'master-agent-dashboard' && maSession) loadMasterAgentStats();
  ...
  // ── Super Agent Session ────────────────────────────────────────
  let saSession = null; // { company_code, agent_code }
  let maSession = null; // { company_code, agent_code }

  // ── Super Agent Login ──────────────────────────────────────────
  window.superAgentLogin = async function () {
    const companyCode = (document.getElementById('saCompanyCode')?.value || '').trim().toUpperCase();
    const agentCode   = (document.getElementById('saAgentCode')?.value   || '').trim().toUpperCase();
    const errEl = document.getElementById('saAgentCodeError');
    if (errEl) errEl.textContent = '';

    if (!companyCode || !agentCode) {
      if (errEl) errEl.textContent = 'Both codes are required';
      return;
    }
    ...
    try {
      const { data, error } = await sb
        .from('agent_registry')
        .select('agent_code, agent_type')
        .eq('company_code', companyCode)
        .eq('agent_code', agentCode)
        .eq('agent_type', 'super_agent')
        .single();

      if (error || !data) throw new Error('Super Agent code not found. Check your codes.');

      saSession = { company_code: companyCode, agent_code: agentCode };
      if (typeof showLoading === 'function') showLoading(false);
  ...
  async function openSuperAgentDashboard() {
    if (!saSession) return;
    const titleEl    = document.getElementById('saDashTitle');
    const subtitleEl = document.getElementById('saDashSubtitle');
    if (titleEl)    titleEl.textContent    = saSession.agent_code + ' Dashboard';
    if (subtitleEl) subtitleEl.textContent = 'Company: ' + saSession.company_code;
    if (typeof showScreen === 'function') showScreen('super-agent-dashboard');
    await loadSuperAgentStats();
  }
  ...
  async function loadSuperAgentStats() {
    ...
    try {
      const { data, error } = await sb
        .from('agent_submissions')
        .select('state')
        .eq('company_code', saSession.company_code)
        .eq('super_agent_code', saSession.agent_code);
      if (error) throw error;

      const states = [...new Set(data.map(r => r.state).filter(Boolean))];
      const totalEl  = document.getElementById('saStatTotal');
      const statesEl = document.getElementById('saStatStates');
      if (totalEl)  totalEl.textContent  = data.length;
      if (statesEl) statesEl.textContent = states.length;
      ...
      data.forEach(r => { if (r.state) breakdown[r.state] = (breakdown[r.state] || 0) + 1; });
      const sorted = Object.entries(breakdown).sort((a, b) => b[1] - a[1]);
      const breakdownEl = document.getElementById('saStateBreakdown');
      if (breakdownEl) {
        breakdownEl.innerHTML = sorted.length === 0
          ? '<em>No submissions yet from your agents.</em>'
          : sorted.map(([state, count]) =>
              `<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #f3f4f6;">
                <span>${state}</span><span style="font-weight:600;color:#0ea5e9;">${count} agent${count > 1 ? 's' : ''}</span>
              </div>`).join('');
      }
    } catch (err) { console.warn('SA stats error:', err.message); }
  }
  ...
  async function loadSuperAgentSubmissions() {
    ...
    try {
      ...
      if (error) throw error;
      if (!data || data.length === 0) { listEl.innerHTML = '<em>No submissions yet from your agents.</em>'; return; }
      listEl.innerHTML = renderSubmissionCards(data);
    } catch (err) { listEl.innerHTML = '<em style="color:#ef4444;">Failed to load: ' + err.message + '</em>'; }
  }

  window.saDownloadReport = async function () {
    if (!saSession) return;
    const sb = getSupabase();
    if (typeof showLoading === 'function') showLoading(true, 'Preparing...');
    try {
      const { data, error } = await sb
        .from('agent_submissions').select('*')
        .eq('company_code', saSession.company_code)
        .eq('super_agent_code', saSession.agent_code)
        .order('state', { ascending: true });
      if (error) throw error;
      downloadCSV(data, saSession.agent_code + '_agents');
      if (typeof showLoading === 'function') showLoading(false);
    } catch (err) {
      if (typeof showLoading === 'function') showLoading(false);
      if (typeof showToast === 'function') showToast('Export failed: ' + err.message, 'error');
  ...
  window.masterAgentLogin = async function () {
    ...
    const agentCode   = (document.getElementById('maAgentCode')?.value   || '').trim().toUpperCase();
    const errEl = document.getElementById('maAgentCodeError');
    if (errEl) errEl.textContent = '';
    ...
    try {
      ...
      if (error || !data) throw new Error('Master Agent code not found. Check your codes.');

      maSession = { company_code: companyCode, agent_code: agentCode };
      if (typeof showLoading === 'function') showLoading(false);
  ...
  async function openMasterAgentDashboard() {
    if (!maSession) return;
    const titleEl    = document.getElementById('maDashTitle');
    const subtitleEl = document.getElementById('maDashSubtitle');
    if (titleEl)    titleEl.textContent    = maSession.agent_code + ' — Regional Dashboard';
    if (subtitleEl) subtitleEl.textContent = 'Company: ' + maSession.company_code;
    if (typeof showScreen === 'function') showScreen('master-agent-dashboard');
    await loadMasterAgentStats();
  }
  ...
  async function loadMasterAgentStats() {
    ...
    try {
      const { data, error } = await sb
        .from('agent_submissions')
        .select('state, super_agent_code')
        .eq('company_code', maSession.company_code)
        .eq('master_agent_code', maSession.agent_code);
      if (error) throw error;

      const totalEl  = document.getElementById('maStatTotal');
      const superEl  = document.getElementById('maStatSuperAgents');
      const supers   = [...new Set(data.map(r => r.super_agent_code).filter(Boolean))];
      if (totalEl) totalEl.textContent = data.length;
      if (superEl) superEl.textContent = supers.length;

      // Super agent breakdown
      const breakdown = {};
      data.forEach(r => {
        const s = r.super_agent_code || '(No Super Agent)';
        breakdown[s] = (breakdown[s] || 0) + 1;
      });
      const sorted = Object.entries(breakdown).sort((a, b) => b[1] - a[1]);
      const breakdownEl = document.getElementById('maSuperAgentBreakdown');
      if (breakdownEl) {
        breakdownEl.innerHTML = sorted.length === 0
          ? '<em>No submissions yet in your region.</em>'
          : sorted.map(([sup, count]) =>
              `<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid #f3f4f6;">
                <div>
                  <div style="font-size:13px;font-weight:600;">⭐ ${sup}</div>
                  <div style="font-size:11px;color:#687076;">${count} agent submission${count > 1 ? 's' : ''}</div>
                </div>
              </div>`).join('');
      }
    } catch (err) { console.warn('MA stats error:', err.message); }
  }
  ...
  async function loadMasterAgentSubmissions() {
    ...
    try {
      const { data, error } = await sb
        ...
        .order('created_at', { ascending: false });
      if (error) throw error;
      if (!data || data.length === 0) { listEl.innerHTML = '<em>No submissions yet in your region.</em>'; return; }
      listEl.innerHTML = renderSubmissionCards(data);
    } catch (err) { listEl.innerHTML = '<em style="color:#ef4444;">Failed to load: ' + err.message + '</em>'; }
  }

  window.maDownloadReport = async function () {
    if (!maSession) return;
    const sb = getSupabase();
    if (typeof showLoading === 'function') showLoading(true, 'Preparing...');
    try {
      ...
      if (error) throw error;
      downloadCSV(data, maSession.agent_code + '_region');
      if (typeof showLoading === 'function') showLoading(false);
    } catch (err) {
      if (typeof showLoading === 'function') showLoading(false);
      if (typeof showToast === 'function') showToast('Export failed: ' + err.message, 'error');
        ...
        <div style="background:#141720;border:1px solid #1e2235;border-radius:14px;padding:16px;margin-bottom:10px;">
          <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:10px;">
            <div>
              ...
              <div style="font-family:'DM Mono',monospace;font-size:10px;color:#4b5472;">Code: #${sub.agent_code}</div>
            </div>
            <span style="font-size:10px;font-weight:600;background:rgba(124,92,252,0.1);border:1px solid rgba(124,92,252,0.2);color:#a78bfa;padding:3px 9px;border-radius:20px;">${sub.state||'—'}</span>
          </div>
          <div style="background:#0f1117;border:1px solid #1e2235;border-radius:8px;padding:8px 12px;margin-bottom:10px;display:flex;gap:12px;flex-wrap:wrap;align-items:center;">
            <div style="font-size:11px;color:#f0f2ff;">${typeBadge}</div>
            ${sub.super_agent_code  ? `<div style="font-size:11px;color:#22d97a;">⭐ ${sub.super_agent_code}</div>`  : ''}
            ${sub.master_agent_code ? `<div style="font-size:11px;color:#a78bfa;">🔷 ${sub.master_agent_code}</div>` : ''}
          </div>
          <div style="display:flex;gap:14px;margin-bottom:8px;">
            <div style="font-size:11px;color:#8892a4;">📍 ${sub.city||'—'}</div>
            <div style="font-size:11px;color:#8892a4;">📞 ${sub.phone||'—'}</div>
          </div>
          <div style="background:#0f1117;border:1px solid #1e2235;border-radius:8px;padding:9px 12px;font-size:10px;color:#8892a4;line-height:1.5;margin-bottom:8px;">${sub.sizing_summary||'—'}</div>
  ...
  function downloadCSV(data, filename) {
    const headers = ['Agent Type','Master Agent Code','Super Agent Code','Agent Code','Agent Name','Phone','State','City','Inverter (kVA)','Panels','Daily Load (kWh)','Date'];
    const rows = data.map(r => {
      const s = r.sizing_json || {};
      return [r.agent_type||'', r.master_agent_code||'', r.super_agent_code||'', r.agent_code||'',
        r.agent_name||'', r.phone||'', r.state||'', r.city||'',
