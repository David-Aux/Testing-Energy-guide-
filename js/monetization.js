...
// Payment temporarily disabled — all leads are free to unlock.
async function egUnlockContact(leadId, role, onUnlocked) {
  // Log the unlock in Supabase for record-keeping (amount_paid = 0)
  ...
  // showToast lives inside the platform.js closure — use the window-pinned version.
  const _toast = window.showToast || function(msg) { console.log(msg); };
  _toast('Contact unlocked!', 'success');

  // Trigger re-render so the contact details appear immediately.
  if (typeof onUnlocked === 'function') {
    onUnlocked(leadId);
  } else {
    // Fallback in case no callback was passed
    if (role === 'installer' && typeof window.renderJobLeads === 'function') window.renderJobLeads();
    if (role === 'vendor'    && typeof window.renderVendorRequests === 'function') window.renderVendorRequests();
  }
}
