    ...
    el.innerHTML=
      '<table style="width:100%;border-collapse:collapse;font-size:12px;">'+
        '<thead><tr style="color:var(--muted,#94a3b8);font-size:11px;">'+
          '<th style="text-align:left;padding:6px 8px;font-weight:600;">Location</th>'+
        ...
        '<tbody>'+
          '<tr style="border-top:1px solid var(--border,#334155);">'+
            '<td style="padding:9px 8px;font-size:11px;color:var(--muted,#94a3b8);">PV → Inverter (DC)</td>'+
          ...
          '</tr>'+
          '<tr style="border-top:1px solid var(--border,#334155);">'+
            '<td style="padding:9px 8px;font-size:11px;color:var(--muted,#94a3b8);">Inverter Output (AC)</td>'+
  ...
  function l4i__renderSoilSelector(){
    ...
    el.innerHTML='<label style="font-size:12px;color:var(--muted);display:block;margin-bottom:6px">Select your soil type to calculate earthing system:</label>'+
      '<select id="l4i-soil-select" onchange="l4i__onSoilChange(this.value)" style="width:100%;padding:10px 12px;border:1px solid var(--border,#e5e7eb);border-radius:8px;font-size:13px;background:var(--bg,#0f172a);color:var(--text,#f1f5f9)">'+
  ...
  function l4i__renderEarth(s){
    ...
    if(!el)return;
    if(!s.earth){el.innerHTML='<div style="font-size:12px;color:var(--muted);padding:8px 0">Select soil type above to calculate your earthing system.</div>';return;}
    var e=s.earth;
    var warnHtml=e.warn?'<div style="background:#fff7ed;border:1px solid #fb923c;border-radius:8px;padding:10px 12px;margin-top:10px;font-size:12px;color:#9a3412;line-height:1.5">⚠️ <strong>High resistivity soil ('+e.rho+' Ω·m).</strong> '+e.rods+' rods achieve '+e.rn+'Ω — above 5Ω target. Consider chemical earthing compound.</div>':'';
    el.innerHTML=
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:8px">'+
        ...
        '<div style="background:var(--panel,#1e293b);border-radius:8px;padding:12px">'+
          ...
          '<div style="font-size:11px;color:var(--muted);margin-top:2px">'+e.soil+' (ρ='+e.rho+' Ω·m) · Target: ≤5Ω</div>'+
        '</div>'+
      '</div>'+
      '<div style="background:var(--panel,#1e293b);border-radius:8px;padding:12px;margin-top:10px">'+
        ...
        '<div style="font-size:13px;font-weight:600">'+e.condS+'mm² Bare Copper · '+e.condRun+'m per rod | IEC 60364-5-54</div>'+
      '</div>'+warnHtml;
  }
  ...
  function l4i__renderBOM(s){
    ...
    if(tot)tot.textContent=l4i__fmt(s.total);
  }
  ...
  function l4i_qtyConfirm(name){
    ...
    l4i_appliances.push({id:newId,name:p.name,watts:p.watts,qty:qty,hours:l4i_getDefaultHours(p.category),inductive:p.inductive,dutyCycle:p.dutyCycle});
    l4i_renderList();
    setTimeout(function(){
  ...
  function l4i_calculate(){if(!l4i_appliances.length)return;l4i__egS=l4i__runEngine(l4i_appliances);l4i__applyPanelMode(l4i__egS,'B');l4i__egS=l4i__buildBOM(l4i__egS,0);l4i__renderResults(l4i__egS);
    try {
