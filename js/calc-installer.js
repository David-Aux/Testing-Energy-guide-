  ...
  function l4i__runEngine(apps){
    ...
    var DB_MAX_KVA=l4i_INVERTER_DB[l4i_INVERTER_DB.length-1].kva;
    ...
    // 12V → 24V escalation
    var V12_CEILING=3.84; // 12V ceiling = 300Ah pack (largest practical single 12V)
  ...
  function l4i__renderResults(s){
    ...
    l4i__applyPanelMode(s,'C'); // Battery-led sizing — Mode C
    var b0=s.opts[s.selBat||0]||s.opts[0]||{};
    // Simple cards
    set('s-inverter-kva',s.inv.kva+' kVA · '+s.V+'V');
    ...
    // Oversize banner
    var ob=document.getElementById('l4i-r-oversize-banner');
    if(ob){
      if(s.oversizeKva){
        ...
        ob.innerHTML='<strong style="display:block;margin-bottom:4px">🔀 Parallel Inverter Configuration Required</strong>'
          +'Load requires ~<strong>'+s.reqKva.toFixed(1)+'kVA</strong>. This exceeds the largest single-unit inverter available. '
          +'Specify a parallel inverter arrangement and battery bank configuration to suit the site.'
          +ciSuggest;
  ...
  function l4i_updateHours(id,val){var h=Math.min(24,Math.max(0.5,parseFloat(val)||1));var a=l4i_appliances.find(x=>x.id===id);if(a){a.hours=h;}l4i_renderList();}
