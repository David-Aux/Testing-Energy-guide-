  ...
  function l4v__runEngine(apps){
    ...
    var DB_MAX_KVA=l4v_INVERTER_DB[l4v_INVERTER_DB.length-1].kva;
    var oversizeKva=(reqKva>DB_MAX_KVA);
    ...
    if(V===12 && reqKwh>V12_CEILING){
      var inv24=l4v_INVERTER_DB.filter(function(i){return i.v===24;}).find(function(i){return i.kva>=inv.kva;})||l4v_INVERTER_DB.filter(function(i){return i.v===24;})[0];
    ...
    if(V===24 && reqKwh>V24_CEILING){
      var inv48=l4v_INVERTER_DB.filter(function(i){return i.v===48;}).find(function(i){return i.kva>=inv.kva;})||l4v_INVERTER_DB.filter(function(i){return i.v===48;})[0];
    ...
    var PSH=5,PR=0.85,PW=l4v_PANEL.wattage,Vmp=l4v_PANEL.vmp,Imp=l4v_PANEL.imp;
  ...
  function l4v__renderResults(s){
    ...
    l4v__applyPanelMode(s,'C'); // Battery-led sizing — Mode C
    var b0=s.opts[s.selBat||0]||s.opts[0]||{};
    // Simple cards
    set('l4v-s-inverter-kva',s.inv.kva+' kVA · '+s.V+'V');
    ...
    set('l4v-s-battery-main',b0.multiPack?('~'+b0.reqKwh+'kWh required'):((b0.units||1)+' × '+(b0.label||'-')));
    set('l4v-s-battery-config',b0.multiPack?'Multi-pack — installer will advise':(b0.wiring||(b0.label||'-')));
    set('l4v-s-battery-alt',b0.multiPack?('~'+b0.reqKwh+' kWh total'):((b0.totalKwh||'-')+' kWh total'));
    set('l4v-s-panels-mode','Battery-led sizing • '+N(s.reqW)+'W required');
    // Load
    ...
    // Oversize banner
    var ob=document.getElementById('l4v-r-oversize-banner');
    if(ob){
      if(s.oversizeKva){
        var ciSuggest=s.reqKva>=30?'<div style="margin-top:8px;padding-top:8px;border-top:1px solid rgba(99,102,241,0.3);font-size:11px;color:#a5b4fc;">⚡ At '+s.reqKva.toFixed(1)+'kVA this is three-phase territory. <button onclick="egCIHandoff('+s.reqKva.toFixed(1)+','+((s.dw/1000).toFixed(1))+')" style="background:#6366f1;border:none;border-radius:6px;color:white;font-size:11px;font-weight:700;padding:4px 10px;cursor:pointer;margin-left:4px;">Open C&amp;I Calculator →</button></div>':'';
        ob.innerHTML='<strong style="display:block;margin-bottom:4px">🔀 Parallel Inverter Configuration Required</strong>'
          +'This system requires ~<strong>'+s.reqKva.toFixed(1)+'kVA</strong>. This exceeds the largest single-unit inverter available. '
          +'The installer will advise on the parallel configuration and battery bank arrangement.'
          +ciSuggest;
    ...
    var tog=document.getElementById('l4v-tech-toggle'); if(tog)tog.style.display='block';
    if(typeof egInitTechSpec==='function') egInitTechSpec('l4v');
  ...
  function l4v__applyPanelMode(s,mode){
    ...
    s.gen=+((panels*l4v_PANEL.wattage*(s.PSH||5)*(s.PR||0.85))/1000).toFixed(2);
    return s;
  }
  ...
  function l4v__renderEarth(s){
    ...
    if(!s.earth){
      ...
      if(typeof egSetSimpleEarthWarn==='function') egSetSimpleEarthWarn('l4v',false,'');
      return;
    }
    var e=s.earth;
    if(typeof egSetSimpleEarthWarn==='function') egSetSimpleEarthWarn('l4v',!!e.warn,e.soil||'');
    var warnHtml=e.warn
  ...
  function l4v_egPickBat(i){
    ...
    if(b){function set(id,v){var el=document.getElementById(id);if(el)el.textContent=v;}set('l4v-s-battery-main',b.multiPack?('~'+b.reqKwh+'kWh required'):((b.units||1)+' × '+b.label));set('l4v-s-battery-config',b.multiPack?'Multi-pack — installer will advise':b.wiring);set('l4v-s-battery-alt',b.multiPack?('~'+b.reqKwh+' kWh total'):(b.totalKwh+' kWh total'));}
  ...
  function l4v_calculate(){if(!l4v_appliances.length)return;l4v__egS=l4v__runEngine(l4v_appliances);l4v__applyPanelMode(l4v__egS,'C');l4v__egS=l4v__buildBOM(l4v__egS,0);l4v__renderResults(l4v__egS);
    try {
